import { V as j } from "./vacuum-of-the-void-DKsyWtj1.mjs";
class w extends Application {
  ...
  _bindRollSheetListeners() {
    ...
    const u = e.querySelector(".roll-sheet-morale-wrap"), l = Math.min(3, Math.max(0, parseInt(((_ = u == null ? void 0 : u.dataset) == null ? void 0 : _.moraleMax) ?? "3", 10) || 0)), r = e.querySelector("input[name='moraleDice']"), d = e.querySelectorAll(".roll-sheet-morale-d8"), m = (s) => {
      let a = Math.min(3, Math.max(0, parseInt(s, 10) || 0));
      a = Math.min(a, l), r && (r.value = String(a)), d.forEach((h, v) => {
        h.classList.toggle("selected", v < a), h.style.pointerEvents = v < l ? "" : "none", h.style.opacity = v < l ? "" : "0.35";
      });
    };
  ...
  _updateFormulaPreview(t) {
    ...
    if (!n) return;
    const e = parseInt(((S = t.querySelector("input[name='advantage']")) == null ? void 0 : S.value) ?? "0", 10) || 0, o = parseInt(((f = t.querySelector("input[name='moraleDice']")) == null ? void 0 : f.value) ?? "0", 10) || 0, i = parseInt(((p = t.querySelector("input[name='constantBonus']")) == null ? void 0 : p.value) ?? "0", 10) || 0, c = parseInt(((M = t.dataset) == null ? void 0 : M.baseMod) ?? "0", 10) || 0, u = parseInt(((g = t.dataset) == null ? void 0 : g.injuryMod) ?? "0", 10) || 0, l = parseInt(((_ = t.dataset) == null ? void 0 : _.injuryPenalty) ?? "0", 10) || 0, r = c + u + l + i, d = [];
  ...
  async _doRoll(t) {
    ...
    const n = this.actor;
    if (!n) return;
    const e = t.querySelector("[name='moraleDice']"), o = t.querySelector("[name='advantage']"), i = t.querySelector("[name='constantBonus']"), c = Math.min(3, Math.max(0, parseInt((e == null ? void 0 : e.value) ?? "0", 10) || 0)), u = Math.max(0, Number(((v = (h = n.system) == null ? void 0 : h.resources) == null ? void 0 : v.morale) ?? 0) || 0), l = Math.min(c, u), r = Math.min(3, Math.max(-3, parseInt((o == null ? void 0 : o.value) ?? "0", 10) || 0)), d = parseInt((i == null ? void 0 : i.value) ?? "0", 10) || 0, m = this._baseModifier, S = this._injuryModifier, f = this._injuryPenalty || 0, p = m + S + f + d, M = [
      "3d6",
    ...
    const _ = g.total, s = _ >= 18 ? "critical" : _ <= 3 ? "fumble" : null, a = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
...
function P(y, t, n) {
  var u, l;
  if (!y) return;
  const e = Number(((l = (u = y.system) == null ? void 0 : u.skills) == null ? void 0 : l[t]) ?? 0) || 0, o = j.BODY_PART_BY_SKILL[t] ? j._getSkillHealthStatusStatic(y, t) : void 0;
  let i = 0;
...
//# sourceMappingURL=roll-sheet-CcLEyAaN.mjs.map
