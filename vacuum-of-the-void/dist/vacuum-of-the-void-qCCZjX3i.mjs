...
const F = class F extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(r) {
    var ke, ve, Ie, Se, qe, Te, De, _e, Oe, Ae, Le, Fe, Ee, Ne, $e, Ce, xe;
    ...
    const d = this.actor.system.weapons ?? {}, g = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], v = (d.slotOrder ?? "").trim(), w = v ? v.split(/\s*,\s*/).filter((m) => g.includes(m)) : [], L = (De = this.actor.system.gear) == null ? void 0 : De.microchips, O = L && typeof L == "object" && !Array.isArray(L) ? L : {}, S = (((_e = O.slot1) == null ? void 0 : _e.itemId) ?? "").trim(), _ = (((Oe = O.slot2) == null ? void 0 : Oe.itemId) ?? "").trim(), E = (((Ae = O.slot3) == null ? void 0 : Ae.itemId) ?? "").trim(), P = this.actor.items.contents.filter((m) => m.type !== "ability").slice().sort((m, k) => {
    ...
    e.actionsTable = P;
    const K = (((Le = this.actor.items) == null ? void 0 : Le.contents) ?? []).filter((m) => m.type === "weapon"), ue = K.map((m) => ({ id: m.id, name: m.name, img: m.img }));
    e.weaponItems = ue;
    ...
    const fe = "— Nincs Mikro-Chip —", te = (m) => {
      const k = [];
      return m !== 1 && k.push(S), m !== 2 && k.push(_), m !== 3 && k.push(E), k;
    };
  ...
  _attachFrameListeners(r) {
    ...
    n && i({ currentTarget: n }), this._writeEffectiveGivenSpeed(r), this._writeEffectiveGivenSpeed(e), requestAnimationFrame(() => this._writeEffectiveGivenSpeed()), setTimeout(() => this._writeEffectiveGivenSpeed(), 100), t.on("click", ".karakter-portrait-img", (a) => {
      ...
      var g;
      a.preventDefault();
      const s = a.currentTarget, l = s.dataset.skill, u = ((g = s.textContent) == null ? void 0 : g.trim()) || l, { openRollSheetForSkill: d } = await import("./roll-sheet-CUYgS2d5.mjs");
      d(this.actor, l, u);
    ...
    t.on("click", ".karakter-inventory-item-name", (a) => {
      ...
      if ((d == null ? void 0 : d.type) === "weapon") {
        const v = u.system.weapons ?? {}, w = (v.slotOrder ?? "").trim(), L = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], O = w ? w.split(/\s*,\s*/).filter((_) => L.includes(_)) : [], S = O.find((_) => {
          var E;
          return (((E = v[_]) == null ? void 0 : E.itemId) ?? "").trim() === l;
        });
        if (S) {
      ...
      if (s.checked) {
        ...
        const _ = {
          "system.weapons.slotOrder": v.filter((E) => E !== w).join(",")
        };
      ...
      if (!d) return;
      const g = (s.dataset.itemId ?? "").trim(), v = (s.dataset.itemName ?? ((E = s.textContent) == null ? void 0 : E.trim()) ?? "").trim() || "— Nincs Mikro-Chip —", w = (s.dataset.itemImg ?? "").trim(), L = l == null ? void 0 : l.querySelector(".karakter-microchip-slot-display-text");
      L && (L.textContent = v);
      let O = l == null ? void 0 : l.querySelector(".karakter-microchip-slot-thumb");
      w ? (O || (O = document.createElement("img"), O.className = "karakter-microchip-slot-thumb", O.alt = "", u == null || u.insertBefore(O, L)), O.src = w, O.style.display = "") : O && O.remove(), l == null || l.classList.remove("karakter-microchip-slot-open");
    ...
    if (e) {
      ...
      if (u) {
        ...
        if (d && d.documentName === "Item" && d.type === "ability") {
          ...
          const L = v === "active" ? "active" : "passive", O = Array.isArray((l = w[L]) == null ? void 0 : l.items) ? w[L].items.slice() : Array.isArray(w[L]) ? w[L].slice() : [];
  ...
  async _rollWeapon(r) {
    var w;
    const t = (this.actor.system.weapons ?? {})[r] ?? {}, i = this.actor.system.skills ?? {}, n = this.actor.items.filter((L) => L.type === "weapon");
    let o = null;
...
Hooks.once("init", () => {
  ...
  Hooks.on("updateActor", (o, c, p, y) => {
    ...
    const v = (E = game.votv) == null ? void 0 : E._lastKarakterSheetBlurSave, w = v && Date.now() - v.at < A ? v.appId : null;
    setTimeout(() => {
      for (const N of l)
        !(N != null && N.rendered) || typeof N.render != "function" || N.id !== w && N.render(!0);
    }, 0);
  });
});
Hooks.on("ready", () => {
...
//# sourceMappingURL=vacuum-of-the-void-qCCZjX3i.mjs.map
