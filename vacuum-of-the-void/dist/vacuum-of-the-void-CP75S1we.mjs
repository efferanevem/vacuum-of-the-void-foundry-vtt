...
const B = class B extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(s) {
    ...
    for (const f of u) {
      const D = ((ce = d[f]) == null ? void 0 : ce.value) ?? 0, _ = c[f] ?? 0;
    ...
    const b = this.actor.system.weapons ?? {}, v = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], I = (b.slotOrder ?? "").trim(), O = I ? I.split(/\s*,\s*/).filter((f) => v.includes(f)) : [], q = (je = this.actor.system.gear) == null ? void 0 : je.microchips, C = q && typeof q == "object" && !Array.isArray(q) ? q : {}, T = (((Ke = C.slot1) == null ? void 0 : Ke.itemId) ?? "").trim(), S = (((Le = C.slot2) == null ? void 0 : Le.itemId) ?? "").trim(), A = (((_e = C.slot3) == null ? void 0 : _e.itemId) ?? "").trim(), K = this.actor.items.contents.filter((f) => f.type !== "Kepesseg" && f.type !== "ability").slice().sort((f, D) => {
      ...
      for (let De = 0; De < O.length; De++) {
        const Ae = O[De];
        if ((((le = b[Ae]) == null ? void 0 : le.itemId) ?? "") === f.id) {
      ...
      const Pe = D && ((ke = f.system) != null && ke.damage) ? f.system.damage : "", ee = D && typeof ((me = f.system) == null ? void 0 : me.range) == "string" ? (f.system.range || "").trim() : "", he = J ? Number((pe = b[J]) == null ? void 0 : pe.bonus) || 0 : "";
      return {
        id: f.id,
        name: f.name,
        img: f.img,
        ...
        rangeStr: ee,
        slotBonusNum: he,
        quantity: Number(((Ee = f.system) == null ? void 0 : Ee.quantity) ?? 1) || 1
      };
    });
    e.actionsTable = K;
    const H = (((tt = this.actor.items) == null ? void 0 : tt.contents) ?? []).filter((f) => f.type === "Fegyver"), Z = H.map((f) => ({ id: f.id, name: f.name, img: f.img }));
    e.weaponItems = Z;
    const G = "— Nincs fegyver —", V = O.filter((f) => {
      const _ = ((b[f] ?? {}).itemId ?? "").trim();
      return _ ? !!H.find((M) => M.id === _) : !1;
    });
    e.weaponSlots = V.map((f, D) => {
      const _ = b[f] ?? {}, L = (_.itemId ?? "").trim(), M = Z.find((J) => J.id === L);
      return {
        slotKey: f,
        itemId: L,
        slotNum: D + 1,
    ...
    const Ie = (((nt = this.actor.items) == null ? void 0 : nt.contents) ?? []).filter((f) => f.type === "MikroChip"), fe = Ie.map((f) => ({ id: f.id, name: f.name, img: f.img })), Oe = "— Nincs Mikro-Chip —", Ce = ["slot1", "slot2", "slot3"], Se = (f) => {
      var _, L, M;
      const D = [];
    ...
    e.microchipItemsSlot1 = fe.filter((f) => !Se(1).includes(f.id)), e.microchipItemsSlot2 = fe.filter((f) => !Se(2).includes(f.id)), e.microchipItemsSlot3 = fe.filter((f) => !Se(3).includes(f.id)), e.microchipSlots = Ce.map((f) => {
      const D = C[f] ?? {}, _ = (D.itemId ?? "").trim(), L = Ie.find((M) => M.id === _);
      return {
        slotKey: f,
        itemId: _,
        displayName: (L == null ? void 0 : L.name) || D.name || Oe,
        img: (L == null ? void 0 : L.img) || "",
        active: D.active === !0
      };
    }), ["1", "2", "3"].forEach((f) => {
      const D = `slot${f}`, _ = C[D] ?? {}, L = (_.itemId ?? "").trim(), M = fe.find((J) => J.id === L);
      e[`microchip${f}ItemId`] = L, e[`microchip${f}DisplayName`] = (M == null ? void 0 : M.name) || _.name || Oe, e[`microchip${f}Img`] = (M == null ? void 0 : M.img) || "";
    }), e.microchipsTable = Ie.map((f) => {
      var le, be, ke, me;
      const D = f.id;
      let _ = null;
      for (const pe of Ce)
      ...
      const L = !!_, M = ((be = f == null ? void 0 : f.system) == null ? void 0 : be.abilityType) ?? "", J = M === "active" ? "Aktív" : M === "passive" ? "Passzív" : M || "—", Y = M === "active", Pe = Number((ke = f == null ? void 0 : f.system) == null ? void 0 : ke.replaceCost) >= 0 ? Number(f.system.replaceCost) : 0, ee = (((me = f == null ? void 0 : f.system) == null ? void 0 : me.description) ?? "").trim(), he = ee ? ee.length > 60 ? ee.slice(0, 57) + "…" : ee : "—";
      return {
        itemId: D,
        actorId: this.actor.id,
        name: (f == null ? void 0 : f.name) ?? "—",
    ...
    const qe = this.actor.system.abilities ?? {}, k = (((at = this.actor.items) == null ? void 0 : at.contents) ?? []).map((f) => {
      var D, _, L;
      return {
        id: f.id,
        name: f.name,
    ...
    }), F = new Map(k.map((f) => [f.id, f])), x = "— Nincs képesség —", U = (f) => {
      const D = qe[f] ?? {}, _ = (D.itemId ?? "").trim(), L = _ ? F.get(_) : null, M = (L == null ? void 0 : L.description) ?? "", J = (L == null ? void 0 : L.kp) ?? 0;
      return {
        itemId: _,
  ...
  _attachFrameListeners(s) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (i) => {
      ...
      if (!n) return;
      const { openRollSheetForWeapon: a } = await import("./roll-sheet-D0MBm73j.mjs");
      a(this.actor, n);
    ...
    d && o({ currentTarget: d }), this._writeEffectiveGivenSpeed(s), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(s), this._writeEffectiveGivenProtection(e), requestAnimationFrame(() => {
      ...
      var p;
      i.preventDefault();
      const r = i.currentTarget, n = r.dataset.skill, a = ((p = r.textContent) == null ? void 0 : p.trim()) || n, { openRollSheetForSkill: l } = await import("./roll-sheet-D0MBm73j.mjs");
      l(this.actor, n, a);
...
const X = class X extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  _attachFrameListeners(s) {
    ...
    t.on("click", ".karakter-inventory-item-name", (i) => {
      ...
      if (!n || !this.actor) return;
      const { openRollSheetForSkill: l } = await import("./roll-sheet-D0MBm73j.mjs");
      l(this.actor, n, a);
    }), t.on("click", ".karakter-ability-chat", async (i) => {
      var n, a;
      i.preventDefault();
      ...
      if (!r || !this.actor) return;
      const n = ((I = (v = this.actor.system) == null ? void 0 : v.gear) == null ? void 0 : I.microchips) ?? {}, l = ["slot1", "slot2", "slot3"].find((O) => {
        var q;
  ...
  async _onDropItem(s, e) {
    var g, h, i, r, n, a, l, p, b, v, I, O, q, C, T;
...
Hooks.on("ready", () => {
  ...
  document.body.addEventListener("click", async (e) => {
    var b, v, I, O, q, C, T, S, A, N, K, H, Z, G, V;
    const t = (v = (b = e.target) == null ? void 0 : b.closest) == null ? void 0 : v.call(b, ".votv-weapon-damage-chat");
    if (!t || t.disabled) return;
    e.preventDefault(), e.stopPropagation();
    const o = (t.dataset.actorId ?? "").trim(), d = (t.dataset.itemId ?? "").trim(), c = (t.dataset.slotKey ?? "").trim(), u = (t.dataset.targetName ?? "").trim(), y = (t.dataset.hitLocationName ?? "").trim(), m = o ? (I = game.actors) == null ? void 0 : I.get(o) : null;
    if (!m) return;
    let g = d ? ((q = (O = m.items) == null ? void 0 : O.get) == null ? void 0 : q.call(O, d)) ?? ((S = (T = (C = m.items) == null ? void 0 : C.contents) == null ? void 0 : T.find) == null ? void 0 : S.call(T, (we) => we.id === d)) : null;
...
//# sourceMappingURL=vacuum-of-the-void-CP75S1we.mjs.map
