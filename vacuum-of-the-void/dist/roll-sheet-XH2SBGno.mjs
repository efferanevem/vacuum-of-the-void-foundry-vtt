import { V as N, c as K } from "./vacuum-of-the-void-WuLbqj1F.mjs";
class V extends Application {
  ...
  _bindRollSheetListeners() {
    ...
    f && f.addEventListener("input", () => t._updateFormulaPreview(e)), f && f.addEventListener("change", () => t._updateFormulaPreview(e)), t._updateFormulaPreview(e), e._votvRollBound = !0, e.addEventListener("click", (s) => {
      ...
      (w = (v = s.target) == null ? void 0 : v.closest) != null && w.call(v, ".roll-sheet-do-roll") && (s.preventDefault(), s.stopPropagation(), t._doRoll(e));
    });
  }
  ...
  async _doRoll(t) {
    ...
    const a = this.actor;
    if (!a) return;
    const e = t.querySelector("[name='moraleDice']"), o = t.querySelector("[name='advantage']"), u = t.querySelector("[name='constantBonus']"), d = Math.min(3, Math.max(0, parseInt((e == null ? void 0 : e.value) ?? "0", 10) || 0)), m = Math.max(0, Number(((x = (w = a.system) == null ? void 0 : w.resources) == null ? void 0 : x.morale) ?? 0) || 0), i = Math.min(d, m), r = Math.min(3, Math.max(-3, parseInt((o == null ? void 0 : o.value) ?? "0", 10) || 0)), c = parseInt((u == null ? void 0 : u.value) ?? "0", 10) || 0, h = this._baseModifier, f = this._injuryModifier, y = this._injuryPenalty || 0, l = h + f + y + c, p = [
      "3d6",
    ...
    const { resultType: b, label: s } = K(g), n = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
    let M = this._label;
    const v = { resultType: b, critLabel: s };
    if (this._isWeaponAttack && ((B = (q = game.user) == null ? void 0 : q.targets) != null && B.size)) {
      const I = Array.from(game.user.targets)[0], S = (I == null ? void 0 : I.actor) ?? null;
      if (S) {
        const L = ((C = S.system) == null ? void 0 : C.combat) ?? {}, F = Number(L.defense ?? 0) || 0, O = Number(L.defenseBonus ?? 0) || 0, U = Number(L.givenProtection ?? 0) || 0;
        let k = 0;
        if (S.type === "Karakter") {
          const H = (((E = S.items) == null ? void 0 : E.contents) ?? []).filter(
            (j) => {
              var P;
              return j.type === "Pancel" && ((P = j.system) == null ? void 0 : P.equipped) === !0;
            }
          );
          for (const j of H) {
            const P = String(((A = j.system) == null ? void 0 : A.protectionBonus) ?? "").trim().replace(",", "."), $ = Number(P);
            Number.isFinite($) && (k += $);
          }
        }
        const R = F + O + U + k, W = (Number(g.total ?? 0) || 0) >= R, z = S.name ?? I.name ?? "Célpont";
    ...
    if (await g.toMessage({
      ...
      await a.update({ "system.resources.morale": D });
    }
    this.close();
  }
}
function G(_, t, a) {
  var m, i;
  if (!_) return;
  const e = Number(((i = (m = _.system) == null ? void 0 : m.skills) == null ? void 0 : i[t]) ?? 0) || 0, o = N.BODY_PART_BY_SKILL[t] ? N._getSkillHealthStatusStatic(_, t) : void 0;
  let u = 0;
  o === 1 ? u = -6 : o === 2 && (u = -3), new V({
    actor: _,
    skillKey: t,
    label: a || t,
    baseModifier: e,
...
export {
  V as VoidRollSheet,
  G as openRollSheetForSkill,
  J as openRollSheetForWeapon
};
//# sourceMappingURL=roll-sheet-XH2SBGno.mjs.map
