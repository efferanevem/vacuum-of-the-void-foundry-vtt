...
const A = class A extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(i) {
    ...
    const z = (((ve = this.actor.items) == null ? void 0 : ve.contents) ?? []).filter((p) => p.type === "microchip").map((p) => ({ id: p.id, name: p.name, img: p.img })), Ge = "— Nincs Mikro-Chip —", J = (p) => {
      const S = [];
  ...
  _attachFrameListeners(i) {
    ...
    a && r({ currentTarget: a }), this._writeEffectiveGivenSpeed(i), this._writeEffectiveGivenSpeed(e), requestAnimationFrame(() => this._writeEffectiveGivenSpeed()), setTimeout(() => this._writeEffectiveGivenSpeed(), 100), t.on("click", ".karakter-portrait-img", (n) => {
      ...
      var w;
      n.preventDefault();
      const c = n.currentTarget, l = c.dataset.skill, o = ((w = c.textContent) == null ? void 0 : w.trim()) || l, { openRollSheetForSkill: d } = await import("./roll-sheet-cz4XfVr_.mjs");
      d(this.actor, l, o);
    ...
    t.on("click", ".karakter-inventory-item-name", (n) => {
      ...
      if (!d) return;
      const w = (c.dataset.itemId ?? "").trim(), b = (c.dataset.itemName ?? ((E = c.textContent) == null ? void 0 : E.trim()) ?? "").trim() || "— Nincs Mikro-Chip —", I = (c.dataset.itemImg ?? "").trim(), N = l == null ? void 0 : l.querySelector(".karakter-microchip-slot-display-text");
      N && (N.textContent = b);
...
//# sourceMappingURL=vacuum-of-the-void-CrAlfu8t.mjs.map
