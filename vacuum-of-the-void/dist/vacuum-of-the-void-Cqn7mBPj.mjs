...
var et = Reflect.get;
var tt = (L, r, e) => r in L ? Qe(L, r, { enumerable: !0, configurable: !0, writable: !0, value: e }) : L[r] = e;
var j = (L, r, e) => tt(L, typeof r != "symbol" ? r + "" : r, e);
var x = (L, r, e) => et(Ve(L), e, r);
const { HTMLField: rt, NumberField: w, SchemaField: T, StringField: f, BooleanField: P, ArrayField: ee } = foundry.data.fields;
...
const { NumberField: ie, SchemaField: at } = foundry.data.fields;
class st extends it {
  static defineSchema() {
    return {
      ...super.defineSchema(),
...
const { NumberField: nt, StringField: Y } = foundry.data.fields;
...
const { NumberField: ae, StringField: je } = foundry.data.fields;
class lt extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      ...
      cost: new ae({ required: !1, integer: !0, min: 0, initial: 1250 }),
      replaceCost: new ae({ required: !1, integer: !0, min: 0, initial: 3 }),
      quantity: new ae({ required: !1, integer: !0, min: 0, initial: 1 })
    };
  }
}
...
const N = class N extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(r) {
    var be, we, ke, ve, Ie, Se, qe, Te, De, _e, Oe, Ae, Le, Fe, Ee, Ne, Ce;
    let e = await super._prepareContext(r);
    e = e ?? {}, e.actor = e.actor ?? this.actor, e.system = e.system ?? this.actor.system;
    const t = Number((ke = (we = (be = this.actor.system) == null ? void 0 : be.resources) == null ? void 0 : we.stress) == null ? void 0 : ke.value) || 0;
    e.stressOver10 = t > 10;
    const i = this.actor.system.resources ?? {}, n = i.hitLocations ?? {}, o = i.currentHealth ?? {}, c = ["head", "torso", "arms", "legs"];
    e.computedHealthStatus = {};
    for (const m of c) {
      const v = ((ve = n[m]) == null ? void 0 : ve.value) ?? 0, I = o[m] ?? 0;
    ...
    const a = e.computedHealthStatus.legs ?? 3, s = Number(((Se = (Ie = this.actor.system) == null ? void 0 : Ie.combat) == null ? void 0 : Se.givenSpeed) ?? 0) || 0;
    let l;
    ...
    const d = this.actor.system.weapons ?? {}, g = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], k = (d.slotOrder ?? "").trim(), b = k ? k.split(/\s*,\s*/).filter((m) => g.includes(m)) : [], F = (qe = this.actor.system.gear) == null ? void 0 : qe.microchips, A = F && typeof F == "object" && !Array.isArray(F) ? F : {}, _ = (((Te = A.slot1) == null ? void 0 : Te.itemId) ?? "").trim(), O = (((De = A.slot2) == null ? void 0 : De.itemId) ?? "").trim(), E = (((_e = A.slot3) == null ? void 0 : _e.itemId) ?? "").trim(), W = this.actor.items.contents.filter((m) => m.type !== "ability").slice().sort((m, v) => {
      ...
      for (let V = 0; V < b.length; V++) {
        const re = b[V];
        if ((((Z = d[re]) == null ? void 0 : Z.itemId) ?? "") === m.id) {
          S = V + 1, q = ((Q = d[re]) == null ? void 0 : Q.bonus) ?? "", C = re;
          break;
        }
      }
      let M = null;
      I && (_ === m.id ? M = "slot1" : O === m.id ? M = "slot2" : E === m.id && (M = "slot3"));
      const K = v && (($e = m.system) != null && $e.damage) ? m.system.damage : "", z = v && typeof ((xe = m.system) == null ? void 0 : xe.range) == "string" ? (m.system.range || "").trim() : "", J = C ? Number((Pe = d[C]) == null ? void 0 : Pe.bonus) || 0 : "";
      return {
        id: m.id,
        name: m.name,
        img: m.img,
    ...
    e.actionsTable = W;
    const B = (((Oe = this.actor.items) == null ? void 0 : Oe.contents) ?? []).filter((m) => m.type === "Fegyver"), G = B.map((m) => ({ id: m.id, name: m.name, img: m.img }));
    e.weaponItems = G;
    const ce = "— Nincs fegyver —", ue = b.filter((m) => {
      const I = ((d[m] ?? {}).itemId ?? "").trim();
      return I ? !!B.find((q) => q.id === I) : !1;
    });
    e.weaponSlots = ue.map((m, v) => {
      const I = d[m] ?? {}, S = (I.itemId ?? "").trim(), q = G.find((C) => C.id === S);
      return {
        slotKey: m,
        slotNum: v + 1,
        displayName: (q == null ? void 0 : q.name) || I.name || ce,
        img: (q == null ? void 0 : q.img) || "",
        bonus: Number(I.bonus) || 0,
        damage: I.damage ?? ""
      };
    });
    const Ye = { kinetic: "Kinetikus", projectile: "Lövedékes", blade: "Pengés", explosive: "Robbanó", other: "Egyéb" }, Je = { light: "Könnyű", medium: "Közepes", heavy: "Nehéz", thrown: "Dobható" };
    e.weaponsTable = ue.map((m) => {
      const v = d[m] ?? {}, I = (v.itemId ?? "").trim(), S = B.find((Q) => Q.id === I), q = (S == null ? void 0 : S.system) ?? {}, C = typeof q.range == "string" ? (q.range || "").trim() : "", M = q.type ?? "", K = (Ye[M] ?? M) || "—", z = q.size ?? "", J = (Je[z] ?? z) || "", Z = [K, J].filter(Boolean).join(", ") || K || "—";
      return {
        slotKey: m,
        itemId: I,
        name: (S == null ? void 0 : S.name) ?? v.name ?? ce,
    ...
    }), e.armor = Array.isArray((Le = (Ae = this.actor.system) == null ? void 0 : Ae.gear) == null ? void 0 : Le.armor) ? this.actor.system.gear.armor : [], e.generalItems = Array.isArray((Ee = (Fe = this.actor.system) == null ? void 0 : Fe.gear) == null ? void 0 : Ee.generalItems) ? this.actor.system.gear.generalItems : [];
    const de = (((Ne = this.actor.items) == null ? void 0 : Ne.contents) ?? []).filter((m) => m.type === "microchip"), X = de.map((m) => ({ id: m.id, name: m.name, img: m.img }));
    e.microchipsTable = ["1", "2", "3"].map((m) => {
      var K, z;
      const v = `slot${m}`, I = A[v] ?? {}, S = (I.itemId ?? "").trim(), q = de.find((J) => J.id === S), C = ((K = q == null ? void 0 : q.system) == null ? void 0 : K.abilityType) ?? "", M = C === "active" ? "Aktív" : C === "passive" ? "Passzív" : C || "—";
      return {
        slotKey: v,
        itemId: S,
        name: (q == null ? void 0 : q.name) ?? I.name ?? me,
        typeLabel: M,
        description: (((z = q == null ? void 0 : q.system) == null ? void 0 : z.description) ?? "").trim() || "—"
      };
    });
    const me = "— Nincs Mikro-Chip —", te = (m) => {
      const v = [];
      return m !== 1 && v.push(_), m !== 2 && v.push(O), m !== 3 && v.push(E), v;
    };
    e.microchipItemsSlot1 = X.filter((m) => !te(1).includes(m.id)), e.microchipItemsSlot2 = X.filter((m) => !te(2).includes(m.id)), e.microchipItemsSlot3 = X.filter((m) => !te(3).includes(m.id)), ["1", "2", "3"].forEach((m) => {
      const v = `slot${m}`, I = A[v] ?? {}, S = (I.itemId ?? "").trim(), q = X.find((C) => C.id === S);
      e[`microchip${m}ItemId`] = S, e[`microchip${m}DisplayName`] = (q == null ? void 0 : q.name) || I.name || me, e[`microchip${m}Img`] = (q == null ? void 0 : q.img) || "", e[`microchip${m}Active`] = I.active === !0;
    });
    const pe = this.actor.system.abilities ?? {}, Xe = (((Ce = this.actor.items) == null ? void 0 : Ce.contents) ?? []).map((m) => {
      var v, I, S;
      return {
        id: m.id,
        name: m.name,
    ...
    }), fe = new Map(Xe.map((m) => [m.id, m])), Ze = "— Nincs képesség —", ge = (m) => {
      const v = pe[m] ?? {}, I = (v.itemId ?? "").trim(), S = I ? fe.get(I) : null, q = (S == null ? void 0 : S.description) ?? "", C = (S == null ? void 0 : S.kp) ?? 0;
      return {
        itemId: I,
        displayName: (S == null ? void 0 : S.name) || v.name || Ze,
        img: (S == null ? void 0 : S.img) || "",
        description: q,
        kp: C
      };
    ...
    return e.abilityPassiveList = he(ye("passive")), e.abilityActiveList = he(ye("active")), e;
  }
  _attachFrameListeners(r) {
    ...
    n && i({ currentTarget: n }), this._writeEffectiveGivenSpeed(r), this._writeEffectiveGivenSpeed(e), requestAnimationFrame(() => this._writeEffectiveGivenSpeed()), setTimeout(() => this._writeEffectiveGivenSpeed(), 100), t.on("click", ".karakter-portrait-img", (a) => {
      ...
      var g;
      a.preventDefault();
      const s = a.currentTarget, l = s.dataset.skill, u = ((g = s.textContent) == null ? void 0 : g.trim()) || l, { openRollSheetForSkill: d } = await import("./roll-sheet-D7Tw384H.mjs");
      d(this.actor, l, u);
    ...
    t.on("click", ".karakter-inventory-item-name", (a) => {
      ...
      if (s.checked) {
        ...
        let O = k.find((B) => {
          var G;
          return !((G = u[B]) != null && G.itemId);
        });
        const E = new Set(k);
        if (!O) {
          const B = g.find((G) => !E.has(G));
          if (!B) return;
          O = B, k.push(B);
        }
        const D = this.actor.items.get(l), W = {
...
Hooks.once("init", () => {
  ...
  Hooks.on("updateActor", (o, c, p, y) => {
    ...
    if (s && l.some((D) => {
      const W = (D == null ? void 0 : D.form) ?? (D == null ? void 0 : D.element);
...
//# sourceMappingURL=vacuum-of-the-void-Cqn7mBPj.mjs.map
