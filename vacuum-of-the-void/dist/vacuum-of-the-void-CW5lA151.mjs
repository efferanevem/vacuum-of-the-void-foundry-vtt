...
const { NumberField: he, StringField: tt } = foundry.data.fields;
...
const x = class x extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(i) {
    ...
    const h = e.computedHealthStatus.legs ?? 3, r = Number(((Re = (je = this.actor.system) == null ? void 0 : je.combat) == null ? void 0 : Re.givenSpeed) ?? 0) || 0;
    let u;
    ...
    const b = this.actor.system.weapons ?? {}, w = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], S = (b.slotOrder ?? "").trim(), D = S ? S.split(/\s*,\s*/).filter((d) => w.includes(d)) : [], C = (Ue = this.actor.system.gear) == null ? void 0 : Ue.microchips, _ = C && typeof C == "object" && !Array.isArray(C) ? C : {}, k = (((Be = _.slot1) == null ? void 0 : Be.itemId) ?? "").trim(), E = (((He = _.slot2) == null ? void 0 : He.itemId) ?? "").trim(), N = (((Ge = _.slot3) == null ? void 0 : Ge.itemId) ?? "").trim(), wt = this.actor.items.contents.filter((d) => d.type !== "Kepesseg" && d.type !== "ability").slice().sort((d, T) => {
      ...
      let M = null;
      I && (k === d.id ? M = "slot1" : E === d.id ? M = "slot2" : N === d.id && (M = "slot3"));
    ...
    e.actionsTable = wt;
    const me = (((ze = this.actor.items) == null ? void 0 : ze.contents) ?? []).filter((d) => d.type === "Fegyver"), Oe = me.map((d) => ({ id: d.id, name: d.name, img: d.img }));
    e.weaponItems = Oe;
    const qe = "— Nincs fegyver —", It = D.filter((d) => {
    ...
    e.weaponSlots = It.map((d, T) => {
      ...
      return {
        ...
        itemId: O,
        slotNum: T + 1,
        displayName: (L == null ? void 0 : L.name) || I.name || qe,
    ...
    e.weaponsTable = me.map((d) => {
      ...
      let I = null, O = {};
      for (const ye of D)
        if ((((Ze = b[ye]) == null ? void 0 : Ze.itemId) ?? "").trim() === T) {
          I = ye, O = b[ye] ?? {};
          break;
        }
    ...
    const qt = (((Ye = this.actor.items) == null ? void 0 : Ye.contents) ?? []).filter((d) => d.type === "Targy");
    ...
    const _t = (((Qe = this.actor.items) == null ? void 0 : Qe.contents) ?? []).filter((d) => d.type === "Egyeb");
    ...
    const pe = (((Je = this.actor.items) == null ? void 0 : Je.contents) ?? []).filter((d) => d.type === "MikroChip"), le = pe.map((d) => ({ id: d.id, name: d.name, img: d.img })), _e = "— Nincs Mikro-Chip —", De = ["slot1", "slot2", "slot3"], fe = (d) => {
      var I, O, L;
      const T = [];
    ...
    e.microchipItemsSlot1 = le.filter((d) => !fe(1).includes(d.id)), e.microchipItemsSlot2 = le.filter((d) => !fe(2).includes(d.id)), e.microchipItemsSlot3 = le.filter((d) => !fe(3).includes(d.id)), e.microchipSlots = De.map((d) => {
      const T = _[d] ?? {}, I = (T.itemId ?? "").trim(), O = pe.find((L) => L.id === I);
      return {
        slotKey: d,
        itemId: I,
        displayName: (O == null ? void 0 : O.name) || T.name || _e,
        img: (O == null ? void 0 : O.img) || "",
        active: T.active === !0
      };
    }), ["1", "2", "3"].forEach((d) => {
      const T = `slot${d}`, I = _[T] ?? {}, O = (I.itemId ?? "").trim(), L = le.find((K) => K.id === O);
      e[`microchip${d}ItemId`] = O, e[`microchip${d}DisplayName`] = (L == null ? void 0 : L.name) || I.name || _e, e[`microchip${d}Img`] = (L == null ? void 0 : L.img) || "";
  ...
  _attachFrameListeners(i) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (r) => {
      ...
      if (!l) return;
      const { openRollSheetForWeapon: o } = await import("./roll-sheet-CwuH5tHt.mjs");
      o(this.actor, l);
    ...
    p && n({ currentTarget: p }), this._writeEffectiveGivenSpeed(i), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(i), this._writeEffectiveGivenProtection(e), requestAnimationFrame(() => {
      ...
      var g;
      r.preventDefault();
      const u = r.currentTarget, l = u.dataset.skill, o = ((g = u.textContent) == null ? void 0 : g.trim()) || l, { openRollSheetForSkill: c } = await import("./roll-sheet-CwuH5tHt.mjs");
      c(this.actor, l, o);
    ...
    if (e) {
      ...
      if (o) {
        ...
        if (c && c.documentName === "Item" && c.type === "Fegyver") {
          ...
          if (b) {
            const S = ((h = g.system) == null ? void 0 : h.weapons) ?? {}, D = (S.slotOrder ?? "").trim(), C = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], _ = D ? D.split(/\s*,\s*/).filter((E) => C.includes(E)) : [], k = _.filter((E) => {
              var N;
              return (((N = S[E]) == null ? void 0 : N.itemId) ?? "").trim() === w;
            });
            if (k.length > 0) {
              const N = {
  ...
  async _rollSkill(i, e) {
    ...
    const { resultType: y, label: h } = nt(m), r = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
    return m.toMessage({
      speaker: ChatMessage.getSpeaker({ actor: this.actor }),
      flavor: e,
...
j(x, "PARTS", foundry.utils.mergeObject(A(x, x, "PARTS") ?? {}, {
  content: { template: "systems/vacuum-of-the-void/templates/actor/karakter-sheet.hbs" }
})), j(x, "DEFAULT_OPTIONS", foundry.utils.mergeObject(A(x, x, "DEFAULT_OPTIONS"), {
  ...
  window: foundry.utils.mergeObject(((ot = A(x, x, "DEFAULT_OPTIONS")) == null ? void 0 : ot.window) ?? {}, {
    resizable: !0,
    contentClasses: [
      ...((ct = (lt = A(x, x, "DEFAULT_OPTIONS")) == null ? void 0 : lt.window) == null ? void 0 : ct.contentClasses) ?? [],
      "votv-scroll"
    ]
  })
...
j(B, "PARTS", foundry.utils.mergeObject(A(B, B, "PARTS") ?? {}, {
  content: { template: "systems/vacuum-of-the-void/templates/actor/npc-sheet.hbs" }
})), j(B, "DEFAULT_OPTIONS", foundry.utils.mergeObject(A(B, B, "DEFAULT_OPTIONS"), {
...
//# sourceMappingURL=vacuum-of-the-void-CW5lA151.mjs.map
