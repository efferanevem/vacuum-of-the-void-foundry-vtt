...
const x = class x extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(i) {
    ...
    const h = e.computedHealthStatus.legs ?? 3, r = Number(((xe = ($e = this.actor.system) == null ? void 0 : $e.combat) == null ? void 0 : xe.givenSpeed) ?? 0) || 0;
    let u;
  ...
  _attachFrameListeners(i) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (r) => {
      ...
      if (!l) return;
      const { openRollSheetForWeapon: o } = await import("./roll-sheet-ClYxw_5F.mjs");
      o(this.actor, l);
    ...
    p && n({ currentTarget: p }), this._writeEffectiveGivenSpeed(i), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(i), this._writeEffectiveGivenProtection(e), requestAnimationFrame(() => {
      ...
      var g;
      r.preventDefault();
      const u = r.currentTarget, l = u.dataset.skill, o = ((g = u.textContent) == null ? void 0 : g.trim()) || l, { openRollSheetForSkill: c } = await import("./roll-sheet-ClYxw_5F.mjs");
      c(this.actor, l, o);
  ...
  async _rollSkill(i, e) {
    ...
    const { resultType: y, label: h } = st(m), r = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
    return m.toMessage({
      speaker: ChatMessage.getSpeaker({ actor: this.actor }),
      flavor: e,
  ...
  async _rollWeapon(i) {
    var w;
    const t = (this.actor.system.weapons ?? {})[i] ?? {}, n = this.actor.system.skills ?? {}, p = this.actor.items.filter((T) => T.type === "Fegyver");
    let s = null;
...
//# sourceMappingURL=vacuum-of-the-void-GKm2ZAAB.mjs.map
