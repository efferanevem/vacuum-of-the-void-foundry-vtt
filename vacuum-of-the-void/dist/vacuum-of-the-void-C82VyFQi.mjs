...
const { HTMLField: ot, NumberField: w, SchemaField: D, StringField: f, BooleanField: M, ArrayField: V } = foundry.data.fields;
...
const { NumberField: re, SchemaField: ct } = foundry.data.fields;
class ut extends lt {
  static defineSchema() {
    return {
      ...super.defineSchema(),
      attributes: new ct({
        body: new re({ required: !0, integer: !0, min: 3, max: 18, initial: 10 }),
        mind: new re({ required: !0, integer: !0, min: 3, max: 18, initial: 10 }),
        soul: new re({ required: !0, integer: !0, min: 3, max: 18, initial: 10 })
      })
    };
  }
}
const { NumberField: ie, SchemaField: dt, StringField: ae } = foundry.data.fields;
class mt extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      ...
      range: new dt({
        short: new ie({ required: !1, integer: !1, min: 0, initial: 0 }),
        long: new ie({ required: !1, integer: !1, min: 0, initial: 0 })
      }),
      special: new ae({ required: !1, blank: !0, initial: "" }),
      quantity: new ie({ required: !1, integer: !0, min: 0, initial: 1 })
    };
  }
}
...
const F = class F extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async render(r = !1, e = {}) {
    ...
    if (n) {
      ...
      const g = document.activeElement, v = o.contains(g), b = g && (g.tagName === "INPUT" && g.type !== "checkbox" && g.type !== "radio" || g.tagName === "TEXTAREA");
      v && b && (c.focus = {
        name: g.name || null,
        id: g.id || null,
    ...
    const y = this.element, h = this.form ?? this.element, a = c.scrollState, s = c.focus, l = typeof (a == null ? void 0 : a.formScrollTop) == "number" || typeof (a == null ? void 0 : a.windowScrollTop) == "number", u = () => {
      ...
      if (this._applyScrollState(y, a), (s == null ? void 0 : s.name) != null || (s == null ? void 0 : s.id) != null) {
        const b = ((g = h.querySelector) == null ? void 0 : g.call(h, s.id ? `#${CSS.escape(s.id)}` : null)) ?? (s.name ? (v = h.querySelector) == null ? void 0 : v.call(h, `[name="${CSS.escape(s.name)}"]`) : null);
  ...
  _writeEffectiveGivenSpeed(r = null) {
    ...
    const i = ((s = (a = this.actor) == null ? void 0 : a.system) == null ? void 0 : s.resources) ?? {}, n = ((u = (l = i.hitLocations) == null ? void 0 : l.legs) == null ? void 0 : u.value) ?? 0, o = ((d = i.currentHealth) == null ? void 0 : d.legs) ?? 0, c = F._healthStatusFromRatio(o, n), p = Number(((b = (v = (g = this.actor) == null ? void 0 : g.system) == null ? void 0 : v.combat) == null ? void 0 : b.givenSpeed) ?? 0) || 0;
    let y;
  ...
  async _prepareContext(r) {
    var ke, ve, Ie, Se, qe, Te, De, _e, Oe, Ae, Le, Fe, Ne, Ee, $e, Ce, xe;
    ...
    const d = this.actor.system.weapons ?? {}, g = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], v = (d.slotOrder ?? "").trim(), b = v ? v.split(/\s*,\s*/).filter((m) => g.includes(m)) : [], L = (De = this.actor.system.gear) == null ? void 0 : De.microchips, O = L && typeof L == "object" && !Array.isArray(L) ? L : {}, S = (((_e = O.slot1) == null ? void 0 : _e.itemId) ?? "").trim(), _ = (((Oe = O.slot2) == null ? void 0 : Oe.itemId) ?? "").trim(), N = (((Ae = O.slot3) == null ? void 0 : Ae.itemId) ?? "").trim(), P = this.actor.items.contents.filter((m) => m.type !== "ability").slice().sort((m, k) => {
      const I = typeof m.sort == "number" ? m.sort : 0, q = typeof k.sort == "number" ? k.sort : 0;
      return I - q;
    }).map((m) => {
      var Z, Me, Pe, je, He, Re, Ue, Be, Ke;
      const k = m.type === "Fegyver", I = m.type === "microchip";
      let q = 0, T = "", C = null;
      for (let Q = 0; Q < b.length; Q++) {
        const te = b[Q];
        if ((((Z = d[te]) == null ? void 0 : Z.itemId) ?? "") === m.id) {
          q = Q + 1, T = ((Me = d[te]) == null ? void 0 : Me.bonus) ?? "", C = te;
          break;
        }
      }
      let j = null;
      I && (S === m.id ? j = "slot1" : _ === m.id ? j = "slot2" : N === m.id && (j = "slot3"));
      const z = k && ((Pe = m.system) != null && Pe.damage) ? m.system.damage : "", G = k && ((He = (je = m.system) == null ? void 0 : je.range) == null ? void 0 : He.short) != null ? m.system.range.short : null, W = k && ((Ue = (Re = m.system) == null ? void 0 : Re.range) == null ? void 0 : Ue.long) != null ? m.system.range.long : null, J = G != null && W != null ? `${G} / ${W} m` : "", X = C ? Number((Be = d[C]) == null ? void 0 : Be.bonus) || 0 : "";
      return {
        id: m.id,
        name: m.name,
        img: m.img,
    ...
    e.weaponsTable = me.map((m) => {
      var J, X;
      const k = d[m] ?? {}, I = (k.itemId ?? "").trim(), q = K.find((Z) => Z.id === I), T = (q == null ? void 0 : q.system) ?? {}, C = (J = T.range) == null ? void 0 : J.short, j = (X = T.range) == null ? void 0 : X.long, z = C != null && j != null ? `${C} / ${j} m` : "", G = T.type ?? "", W = (et[G] ?? G) || "—";
      return {
        slotKey: m,
        itemId: I,
    ...
    }), e.armor = Array.isArray((Ne = (Fe = this.actor.system) == null ? void 0 : Fe.gear) == null ? void 0 : Ne.armor) ? this.actor.system.gear.armor : [], e.generalItems = Array.isArray(($e = (Ee = this.actor.system) == null ? void 0 : Ee.gear) == null ? void 0 : $e.generalItems) ? this.actor.system.gear.generalItems : [];
    const pe = (((Ce = this.actor.items) == null ? void 0 : Ce.contents) ?? []).filter((m) => m.type === "microchip"), Y = pe.map((m) => ({ id: m.id, name: m.name, img: m.img }));
    e.microchipsTable = ["1", "2", "3"].map((m) => {
      var z, G;
      const k = `slot${m}`, I = O[k] ?? {}, q = (I.itemId ?? "").trim(), T = pe.find((W) => W.id === q), C = ((z = T == null ? void 0 : T.system) == null ? void 0 : z.abilityType) ?? "", j = C === "active" ? "Aktív" : C === "passive" ? "Passzív" : C || "—";
      return {
        slotKey: k,
        itemId: q,
    ...
    const fe = "— Nincs Mikro-Chip —", ee = (m) => {
      const k = [];
      return m !== 1 && k.push(S), m !== 2 && k.push(_), m !== 3 && k.push(N), k;
    };
    e.microchipItemsSlot1 = Y.filter((m) => !ee(1).includes(m.id)), e.microchipItemsSlot2 = Y.filter((m) => !ee(2).includes(m.id)), e.microchipItemsSlot3 = Y.filter((m) => !ee(3).includes(m.id)), ["1", "2", "3"].forEach((m) => {
      const k = `slot${m}`, I = O[k] ?? {}, q = (I.itemId ?? "").trim(), T = Y.find((C) => C.id === q);
  ...
  _attachFrameListeners(r) {
    ...
    n && i({ currentTarget: n }), this._writeEffectiveGivenSpeed(r), this._writeEffectiveGivenSpeed(e), requestAnimationFrame(() => this._writeEffectiveGivenSpeed()), setTimeout(() => this._writeEffectiveGivenSpeed(), 100), t.on("click", ".karakter-portrait-img", (a) => {
      ...
      var g;
      a.preventDefault();
      const s = a.currentTarget, l = s.dataset.skill, u = ((g = s.textContent) == null ? void 0 : g.trim()) || l, { openRollSheetForSkill: d } = await import("./roll-sheet-CCdH8UTk.mjs");
      d(this.actor, l, u);
    ...
    t.on("click", ".karakter-inventory-item-name", (a) => {
      ...
      if ((d == null ? void 0 : d.type) === "Fegyver") {
        const v = u.system.weapons ?? {}, b = (v.slotOrder ?? "").trim(), L = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], O = b ? b.split(/\s*,\s*/).filter((_) => L.includes(_)) : [], S = O.find((_) => {
          var N;
      ...
      if (!l) return;
      const u = this.actor.system.weapons ?? {}, d = (u.slotOrder ?? "").trim(), g = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], v = d ? d.split(/\s*,\s*/).filter((S) => g.includes(S)) : [], b = v.find((S) => {
        var _;
      ...
      if (s.checked) {
        ...
        const _ = {
          "system.weapons.slotOrder": v.filter((N) => N !== b).join(",")
        };
      ...
      if (!d) return;
      const g = (s.dataset.itemId ?? "").trim(), v = (s.dataset.itemName ?? ((N = s.textContent) == null ? void 0 : N.trim()) ?? "").trim() || "— Nincs Mikro-Chip —", b = (s.dataset.itemImg ?? "").trim(), L = l == null ? void 0 : l.querySelector(".karakter-microchip-slot-display-text");
      L && (L.textContent = v);
      ...
      if (!s || !l) return;
      const u = this.actor.system.abilities ?? {}, g = (Array.isArray((v = u[s]) == null ? void 0 : v.items) ? u[s].items.slice() : []).filter((b) => b !== l);
    ...
    if (e) {
      ...
      if (u) {
        ...
        if (d && d.documentName === "Item" && d.type === "ability") {
          ...
          const v = (((a = d.system) == null ? void 0 : a.kind) ?? "passive").toString(), b = this.actor.system.abilities ?? {};
          if (i) {
          ...
          const L = v === "active" ? "active" : "passive", O = Array.isArray((l = b[L]) == null ? void 0 : l.items) ? b[L].items.slice() : Array.isArray(b[L]) ? b[L].slice() : [];
...
const R = class R extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(r) {
    let e = await super._prepareContext(r);
    return e = e ?? {}, e.item = this.document, e.system = this.document.system, e;
  }
};
H(R, "PARTS", foundry.utils.mergeObject(x(R, R, "PARTS") ?? {}, {
  ...
  minWidth: 550,
  height: 400,
  form: foundry.utils.mergeObject(((Ze = x(R, R, "DEFAULT_OPTIONS")) == null ? void 0 : Ze.form) ?? {}, { submitOnChange: !1 })
}));
let oe = R;
var Qe;
...
Hooks.once("init", () => {
  ...
  Hooks.on("updateActor", (o, c, p, y) => {
    ...
    const v = (N = game.votv) == null ? void 0 : N._lastKarakterSheetBlurSave, b = v && Date.now() - v.at < A ? v.appId : null;
    setTimeout(() => {
      for (const E of l)
        !(E != null && E.rendered) || typeof E.render != "function" || E.id !== b && E.render(!0);
    }, 0);
  });
});
Hooks.on("ready", () => {
...
//# sourceMappingURL=vacuum-of-the-void-C82VyFQi.mjs.map
