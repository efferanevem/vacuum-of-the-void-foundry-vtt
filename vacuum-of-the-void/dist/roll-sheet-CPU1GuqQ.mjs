import { V as Y, c as fe } from "./vacuum-of-the-void-SP04zivT.mjs";
class ae extends Application {
  ...
  _getCurrentWeaponBaseModifier() {
    ...
    if (t.type === "Karakter" && m) {
      const d = Y._getSkillHealthStatusStatic(t, m);
      d === 1 ? I = -6 : d === 2 && (I = -3);
    }
    return f + b + I;
  }
  async _doRoll(t) {
    ...
    if (e) {
      ...
      try {
        ...
        const { resultType: ne, label: oe } = fe(C), ie = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
        let le = this._label;
          ...
          if (k) {
            ...
            if (k.type === "Karakter") {
              const T = (((V = k.items) == null ? void 0 : V.contents) ?? []).filter(
                (U) => {
                  var G;
                  return U.type === "Pancel" && ((G = U.system) == null ? void 0 : G.equipped) === !0;
                }
              );
              for (const U of T) {
                const G = String(((M = U.system) == null ? void 0 : M.protectionBonus) ?? "").trim().replace(",", "."), se = Number(G);
                Number.isFinite(se) && (Z += se);
              }
            }
            const he = ue + Z + de + me, ee = re + ce + he, te = (Number(C.total ?? 0) || 0) >= ee, pe = k.name ?? H.name ?? "Célpont";
            let R = null, K = null;
...
function ge(N, t, e) {
  var s, u;
  if (!N) return;
  const a = Number(((u = (s = N.system) == null ? void 0 : s.skills) == null ? void 0 : u[t]) ?? 0) || 0, r = Y.BODY_PART_BY_SKILL[t] ? Y._getSkillHealthStatusStatic(N, t) : void 0;
  let y = 0;
  r === 1 ? y = -6 : r === 2 && (y = -3);
...
function Se(N, t) {
  ...
  let c = 0;
  if (e.type === "Karakter") {
    const C = Y._getSkillHealthStatusStatic(e, n);
    C === 1 ? c = -6 : C === 2 && (c = -3);
  }
...
export {
  ae as VoidRollSheet,
  ge as openRollSheetForSkill,
  Se as openRollSheetForWeapon
};
//# sourceMappingURL=roll-sheet-CPU1GuqQ.mjs.map
