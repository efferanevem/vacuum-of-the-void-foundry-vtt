// Egyéb információk importálása a packs/_source/egyeb-informaciok.json-ből
// az "Egyéb Információk" compendiumba, a JSON-ben megadott "path" alapján mappaszerkezettel.
// Minden elem kényszerítetten "Egyeb" típusúként kerül importálásra.

(async () => {
  const SYSTEM_ID = "vacuum-of-the-void";
  const PACK_NAME = "egyeb-informaciok";
  const SOURCE_PATH = "systems/vacuum-of-the-void/packs/_source/egyeb-informaciok.json";
  ...
  if (!Array.isArray(entries) || !entries.length) {
    ui.notifications.warn("Az egyeb-informaciok.json üres vagy nem tömb.");
    return;
  }
  ...
  for (const raw of entries) {
    ...
    try {
      if (existingItem) {
        ...
        if (createdItem?.name) existingByName.set(createdItem.name, createdItem);
        created++;
      }
    } catch (err) {
  ...
  ui.notifications.info(
    `Kész: ${created} új, ${updated} frissített egyéb információ importálva, ${skipped} kihagyva/hibás.`
  );
})();
