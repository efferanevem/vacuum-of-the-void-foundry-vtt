...
const { NumberField: ge, StringField: et } = foundry.data.fields;
...
const x = class x extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(i) {
    ...
    e.actionsTable = wt;
    const de = (((Ge = this.actor.items) == null ? void 0 : Ge.contents) ?? []).filter((d) => d.type === "Fegyver"), Se = de.map((d) => ({ id: d.id, name: d.name, img: d.img }));
    e.weaponItems = Se;
    ...
    e.weaponsTable = de.map((d) => {
      ...
      let I = null, O = {};
      for (const fe of D)
        if ((((Xe = b[fe]) == null ? void 0 : Xe.itemId) ?? "").trim() === T) {
          I = fe, O = b[fe] ?? {};
          break;
        }
    ...
    const me = (((Qe = this.actor.items) == null ? void 0 : Qe.contents) ?? []).filter((d) => d.type === "MikroChip"), le = me.map((d) => ({ id: d.id, name: d.name, img: d.img })), qe = "— Nincs Mikro-Chip —", _e = ["slot1", "slot2", "slot3"], pe = (d) => {
      var I, O, L;
      const T = [];
    ...
    e.microchipItemsSlot1 = le.filter((d) => !pe(1).includes(d.id)), e.microchipItemsSlot2 = le.filter((d) => !pe(2).includes(d.id)), e.microchipItemsSlot3 = le.filter((d) => !pe(3).includes(d.id)), e.microchipSlots = _e.map((d) => {
  ...
  _attachFrameListeners(i) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (r) => {
      ...
      if (!l) return;
      const { openRollSheetForWeapon: o } = await import("./roll-sheet-BonpWBhy.mjs");
      o(this.actor, l);
    ...
    p && n({ currentTarget: p }), this._writeEffectiveGivenSpeed(i), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(i), this._writeEffectiveGivenProtection(e), requestAnimationFrame(() => {
      ...
      var g;
      r.preventDefault();
      const u = r.currentTarget, l = u.dataset.skill, o = ((g = u.textContent) == null ? void 0 : g.trim()) || l, { openRollSheetForSkill: c } = await import("./roll-sheet-BonpWBhy.mjs");
      c(this.actor, l, o);
...
//# sourceMappingURL=vacuum-of-the-void-BnwXwsKN.mjs.map
