...
const x = class x extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(i) {
    ...
    const me = (((Je = this.actor.items) == null ? void 0 : Je.contents) ?? []).filter((d) => d.type === "MikroChip"), le = me.map((d) => ({ id: d.id, name: d.name, img: d.img })), _e = "— Nincs Mikro-Chip —", De = ["slot1", "slot2", "slot3"], pe = (d) => {
      var I, O, L;
      const T = [];
    ...
    e.microchipItemsSlot1 = le.filter((d) => !pe(1).includes(d.id)), e.microchipItemsSlot2 = le.filter((d) => !pe(2).includes(d.id)), e.microchipItemsSlot3 = le.filter((d) => !pe(3).includes(d.id)), e.microchipSlots = De.map((d) => {
      const T = _[d] ?? {}, I = (T.itemId ?? "").trim(), O = me.find((L) => L.id === I);
      return {
        slotKey: d,
        itemId: I,
  ...
  _attachFrameListeners(i) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (r) => {
      ...
      if (!l) return;
      const { openRollSheetForWeapon: o } = await import("./roll-sheet-BV0QvNzm.mjs");
      o(this.actor, l);
    ...
    p && n({ currentTarget: p }), this._writeEffectiveGivenSpeed(i), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(i), this._writeEffectiveGivenProtection(e), requestAnimationFrame(() => {
      ...
      var g;
      r.preventDefault();
      const u = r.currentTarget, l = u.dataset.skill, o = ((g = u.textContent) == null ? void 0 : g.trim()) || l, { openRollSheetForSkill: c } = await import("./roll-sheet-BV0QvNzm.mjs");
      c(this.actor, l, o);
...
//# sourceMappingURL=vacuum-of-the-void-D0rWXr_c.mjs.map
