...
var Vs = Reflect.get;
var Ws = (S, s, e) => s in S ? Bs(S, s, { enumerable: !0, configurable: !0, writable: !0, value: e }) : S[s] = e;
var se = (S, s, e) => Ws(S, typeof s != "symbol" ? s + "" : s, e);
var U = (S, s, e) => Vs(Gs(S), e, s);
const { HTMLField: Js, NumberField: F, SchemaField: H, StringField: C, BooleanField: pe, ArrayField: It } = foundry.data.fields;
...
const { NumberField: $t, SchemaField: Ys } = foundry.data.fields;
class Xs extends At {
  static defineSchema() {
    return {
      ...super.defineSchema(),
      attributes: new Ys({
        body: new $t({ required: !0, integer: !0, min: 3, max: 18, initial: 10 }),
        mind: new $t({ required: !0, integer: !0, min: 3, max: 18, initial: 10 }),
        soul: new $t({ required: !0, integer: !0, min: 3, max: 18, initial: 10 })
      })
    };
  }
}
const { SchemaField: Qs, StringField: ms, NumberField: Zs } = foundry.data.fields;
class ei extends At {
  static defineSchema() {
    return {
      ...super.defineSchema(),
...
const { BooleanField: ni, NumberField: wi, StringField: Ue } = foundry.data.fields;
...
const { BooleanField: ai, StringField: Ft } = foundry.data.fields;
class ri extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      /** Felszerelve a karakteren (karakter lap checkbox). */
      equipped: new ai({ required: !1, initial: !1 }),
...
const { NumberField: xt, StringField: ys } = foundry.data.fields;
class oi extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      ...
      cost: new xt({ required: !1, integer: !0, min: 0, initial: 1250 }),
      replaceCost: new xt({ required: !1, integer: !0, min: 0, initial: 3 }),
      quantity: new xt({ required: !1, integer: !0, min: 0, initial: 1 })
    };
  }
}
const { NumberField: ci, StringField: gs } = foundry.data.fields;
...
const { StringField: bs } = foundry.data.fields;
class li extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      description: new bs({ required: !1, blank: !0, initial: "" }),
      quantity: new bs({ required: !1, blank: !0, initial: "1" })
    };
  }
}
const { StringField: di } = foundry.data.fields;
class mi extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      description: new di({ required: !1, blank: !0, initial: "" })
    };
  }
}
const { NumberField: Rt, StringField: rt, SchemaField: vs, ArrayField: pi } = foundry.data.fields;
class fi extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      /** Szint (0–3). */
      level: new Rt({ required: !0, integer: !0, min: 0, max: 3, initial: 0 }),
      /** Életerő: max / aktuális. */
      health: new vs({
        max: new Rt({ required: !0, integer: !0, min: 0, initial: 0 }),
        value: new Rt({ required: !0, integer: !0, min: 0, initial: 0 })
      }),
...
const ee = class ee extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(s) {
    ...
    e.weaponItems = ce;
    const ve = "— Nincs fegyver —", D = K.filter((b) => {
    ...
    e.armorTable = Q.map((b) => {
      ...
      return {
        ...
        level: yi((L.level ?? "").toString().trim()),
  ...
  _attachFrameListeners(s) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (i) => {
      ...
      if (!r) return;
      const { openRollSheetForWeapon: u } = await import("./roll-sheet-D08sav7E.mjs");
      u(this.actor, r);
      ...
      i.preventDefault(), i.stopPropagation();
      const { openRollSheetForSkill: o } = await import("./roll-sheet-D08sav7E.mjs");
    ...
    t.on("input change", 'input[name="system.combat.defense"], input[name="system.combat.defenseBonus"], input[name="system.combat.speed"], input[name="system.combat.speedBonus"]', c), this._writeEffectiveGivenSpeed(s), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(s), this._writeEffectiveGivenProtection(e), this._writeTotalDefense(s), this._writeTotalDefense(e), this._writeTotalSpeed(s), this._writeTotalSpeed(e), requestAnimationFrame(() => {
      ...
      var y;
      i.preventDefault();
      const o = i.currentTarget, r = o.dataset.skill, u = ((y = o.textContent) == null ? void 0 : y.trim()) || r, { openRollSheetForSkill: h } = await import("./roll-sheet-D08sav7E.mjs");
      h(this.actor, r, u);
      ...
      if (!y) return;
      const v = ((_ = (T = this.actor.items).get) == null ? void 0 : _.call(T, y)) ?? ((q = (O = this.actor.items.contents) == null ? void 0 : O.find) == null ? void 0 : q.call(O, (K) => K.id === y));
    ...
    if (e) {
      ...
      if (h) {
        ...
        if (y && y.documentName === "Item" && y.type === "Fegyver") {
          ...
          if (k) {
            const T = ((i = v.system) == null ? void 0 : i.weapons) ?? {}, _ = (T.slotOrder ?? "").trim(), O = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], q = _ ? _.split(/\s*,\s*/).filter((K) => O.includes(K)) : [], j = q.filter((K) => {
              var B;
              return (((B = T[K]) == null ? void 0 : B.itemId) ?? "").trim() === w;
            });
            if (j.length > 0) {
              const B = {
  ...
  async _rollSkill(s, e) {
    ...
    const { resultType: m, label: g } = Mt(l), a = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
    return l.toMessage({
      speaker: ChatMessage.getSpeaker({ actor: this.actor }),
      flavor: e,
  ...
  async _rollWeapon(s) {
    ...
    const { resultType: h, label: y } = Mt(u), v = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
    return u.toMessage({
      speaker: ChatMessage.getSpeaker({ actor: this.actor }),
      flavor: `${p} – találat`,
...
se(ee, "PARTS", foundry.utils.mergeObject(
  ...
  U(ee, ee, "DEFAULT_OPTIONS"),
  {
    ...
    minWidth: 1e3,
    height: 800,
    form: foundry.utils.mergeObject(((ks = U(ee, ee, "DEFAULT_OPTIONS")) == null ? void 0 : ks.form) ?? {}, { submitOnChange: !1 }),
    window: foundry.utils.mergeObject(((ws = U(ee, ee, "DEFAULT_OPTIONS")) == null ? void 0 : ws.window) ?? {}, {
      resizable: !0,
      contentClasses: [
        ...((Ss = (Is = U(ee, ee, "DEFAULT_OPTIONS")) == null ? void 0 : Is.window) == null ? void 0 : Ss.contentClasses) ?? [],
        "votv-scroll"
      ]
    })
  },
...
const fe = class fe extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(s) {
    var Me, st, Ge, te, he, ae, be, it, _e, Oe, Et, je, Ve, Dt, _t, ot, Ot, ct, lt, ut, dt, mt, pt, ft, yt, gt, We, ht, bt, Je;
    ...
    e.weaponsTable = o.map((I) => {
      ...
      const ke = !!P, ne = (I == null ? void 0 : I.system) ?? {}, Ce = typeof ne.range == "string" ? (ne.range || "").trim() : "", Te = ne.type ?? "", we = (w[Te] ?? Te) || "—", $e = ne.size ?? "", Ye = (T[$e] ?? $e) || "", Xe = [we, Ye].filter(Boolean).join(", ") || we || "—", Ct = Te === "projectile" || $e === "thrown", qt = ne.quantity != null ? String(ne.quantity).trim() : "1", Nt = String(ne.bonus ?? G.bonus ?? "").trim() || "0", Lt = (ne.damage ?? G.damage ?? "").trim() || "";
      return {
        slotKey: P ?? "",
        itemId: E,
        actorId: t.id,
    ...
    e.armorTable = _.map((I) => {
      ...
      return {
        ...
        level: gi((E.level ?? "").toString().trim()),
    ...
    const D = ((pt = e.system) == null ? void 0 : pt.resources) ?? {}, x = (yt = (ft = e.system) == null ? void 0 : ft.combat) == null ? void 0 : yt.initiativeResult, V = typeof x == "number" ? x : -1, Q = [
      { min: 0, max: 6, kp: 2 },
      { min: 7, max: 12, kp: 3 },
      { min: 13, max: 18, kp: 4 },
      { min: 19, max: 24, kp: 5 },
  ...
  _attachFrameListeners(s) {
    ...
    t.on("click", ".karakter-inventory-item-name", (a) => {
      ...
      if (!o || !this.actor) return;
      const { openRollSheetForWeapon: r } = await import("./roll-sheet-D08sav7E.mjs");
      r(this.actor, o);
      ...
      if (a.preventDefault(), a.stopPropagation(), !this.actor) return;
      const { openRollSheetForSkill: i } = await import("./roll-sheet-D08sav7E.mjs");
      ...
      if (!o || !this.actor) return;
      const { openRollSheetForSkill: u } = await import("./roll-sheet-D08sav7E.mjs");
      u(this.actor, o, r);
  ...
  async _rollWeapon(s) {
    ...
    const { resultType: y, label: v } = Mt(h), k = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
    return h.toMessage({
  ...
  async _onDropItem(s, e) {
    var m, g, a, i, o, r, u, h, y, v, k, w, T, _, O, q, j, K, B;
...
se(fe, "PARTS", foundry.utils.mergeObject(
  ...
  U(fe, fe, "DEFAULT_OPTIONS"),
  {
    ...
    minWidth: 800,
    height: 720,
    form: foundry.utils.mergeObject(((Ts = U(fe, fe, "DEFAULT_OPTIONS")) == null ? void 0 : Ts.form) ?? {}, { submitOnChange: !1 }),
    window: foundry.utils.mergeObject(((Ds = U(fe, fe, "DEFAULT_OPTIONS")) == null ? void 0 : Ds.window) ?? {}, {
      resizable: !0,
      contentClasses: [
        ...((Os = (_s = U(fe, fe, "DEFAULT_OPTIONS")) == null ? void 0 : _s.window) == null ? void 0 : Os.contentClasses) ?? [],
        "votv-scroll"
      ]
    })
  },
...
const ye = class ye extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(s) {
    ...
    const d = (((K = this.actor.items) == null ? void 0 : K.contents) ?? []).filter((D) => D.type === "Pancel");
    e.armorTable = d.map((D) => {
      ...
      return {
        ...
        level: hi((x.level ?? "").toString().trim()),
...
se(ye, "PARTS", foundry.utils.mergeObject(
  ...
  U(ye, ye, "DEFAULT_OPTIONS"),
  {
    ...
    minWidth: 1e3,
    height: 800,
    form: foundry.utils.mergeObject(((Cs = U(ye, ye, "DEFAULT_OPTIONS")) == null ? void 0 : Cs.form) ?? {}, { submitOnChange: !1 }),
    window: foundry.utils.mergeObject(((qs = U(ye, ye, "DEFAULT_OPTIONS")) == null ? void 0 : qs.window) ?? {}, {
      resizable: !0,
      contentClasses: [
        ...((Ls = (Ns = U(ye, ye, "DEFAULT_OPTIONS")) == null ? void 0 : Ns.window) == null ? void 0 : Ls.contentClasses) ?? [],
        "votv-scroll"
      ]
    })
  },
...
se(qe, "PARTS", foundry.utils.mergeObject(U(qe, qe, "PARTS") ?? {}, {
  ...
  minWidth: 550,
  height: 400,
  form: foundry.utils.mergeObject(((Fs = U(qe, qe, "DEFAULT_OPTIONS")) == null ? void 0 : Fs.form) ?? {}, { submitOnChange: !1 })
}));
let zt = qe;
var As;
...
se(Le, "PARTS", foundry.utils.mergeObject(U(Le, Le, "PARTS") ?? {}, {
  ...
  minWidth: 500,
  height: 420,
  form: foundry.utils.mergeObject(((Es = U(Le, Le, "DEFAULT_OPTIONS")) == null ? void 0 : Es.form) ?? {}, { submitOnChange: !1 })
}));
let Ht = Le;
var Ps;
const Fe = class Fe extends foundry.applications.api.HandlebarsApplicationMixin(
...
se(Fe, "PARTS", foundry.utils.mergeObject(U(Fe, Fe, "PARTS") ?? {}, {
  ...
  minWidth: 500,
  height: 420,
  form: foundry.utils.mergeObject(((Ps = U(Fe, Fe, "DEFAULT_OPTIONS")) == null ? void 0 : Ps.form) ?? {}, { submitOnChange: !0 })
}));
let Bt = Fe;
var $s;
...
se(Ae, "PARTS", foundry.utils.mergeObject(U(Ae, Ae, "PARTS") ?? {}, {
  ...
  minWidth: 400,
  height: 360,
  form: foundry.utils.mergeObject((($s = U(Ae, Ae, "DEFAULT_OPTIONS")) == null ? void 0 : $s.form) ?? {}, { submitOnChange: !1 })
}));
let Gt = Ae;
var xs;
const Ee = class Ee extends foundry.applications.api.HandlebarsApplicationMixin(
...
se(Ee, "PARTS", foundry.utils.mergeObject(U(Ee, Ee, "PARTS") ?? {}, {
  ...
  minWidth: 400,
  height: 360,
  form: foundry.utils.mergeObject(((xs = U(Ee, Ee, "DEFAULT_OPTIONS")) == null ? void 0 : xs.form) ?? {}, { submitOnChange: !1 })
}));
let Vt = Ee;
var Rs, Ms;
...
se(Se, "PARTS", foundry.utils.mergeObject(U(Se, Se, "PARTS") ?? {}, {
  ...
  U(Se, Se, "DEFAULT_OPTIONS"),
  {
    ...
    minWidth: 520,
    height: 480,
    form: foundry.utils.mergeObject(((Rs = U(Se, Se, "DEFAULT_OPTIONS")) == null ? void 0 : Rs.form) ?? {}, { submitOnChange: !1 }),
    dragDrop: [
      ...((Ms = U(Se, Se, "DEFAULT_OPTIONS")) == null ? void 0 : Ms.dragDrop) ?? [],
      {
        dragSelector: null,
...
Hooks.once("init", () => {
  ...
  }, (e = CONFIG.Actor).dataModels ?? (e.dataModels = {}), CONFIG.Actor.dataModels.Karakter = Xs, CONFIG.Actor.dataModels.Npc = At, CONFIG.Actor.dataModels.Jarmu = ei, CONFIG.Actor.dataModels.Bazis = ii, (t = CONFIG.Item).dataModels ?? (t.dataModels = {}), CONFIG.Item.dataModels.Fegyver = fs, CONFIG.Item.dataModels.weapon = fs, CONFIG.Item.dataModels.Pancel = ri, CONFIG.Item.dataModels.MikroChip = oi, CONFIG.Item.dataModels.Kepesseg = hs, CONFIG.Item.dataModels.ability = hs, CONFIG.Item.dataModels.Targy = li, CONFIG.Item.dataModels.Egyeb = mi, CONFIG.Item.dataModels.Jarmuegyseg = fi, (n = CONFIG.Actor).trackableAttributes ?? (n.trackableAttributes = {}), CONFIG.Actor.trackableAttributes.Karakter = {
  ...
  Hooks.on("updateActor", (d, l, m, g) => {
    ...
    if (d.type === "Npc" && ((j = l == null ? void 0 : l.system) != null && j.identity) && "size" in l.system.identity) {
      const M = Ks((B = (K = d.system) == null ? void 0 : K.identity) == null ? void 0 : B.size);
      d.update({
...
Hooks.on("ready", () => {
  ...
  document.body.addEventListener("click", async (e) => {
    var h, y, v, k, w, T, _, O, q, j, K, B, R, Y, Z, J, X, M, W, ce, ve, D, x;
...
Hooks.on("preCreateToken", (S, s, e) => {
  ...
  if (f) {
    ...
    if (f.type === "Npc") {
      const m = Ks((l = (d = f.system) == null ? void 0 : d.identity) == null ? void 0 : l.size);
      S.updateSource({
        ...t ? { actorId: t } : {},
        "bar1.attribute": "",
        "bar2.attribute": "resources.health",
...
function zs(S) {
  ...
  for (const t of e) {
    const n = t.actor;
    !n || n.type !== "Karakter" && n.type !== "Npc" || n.update(vi).catch((f) => {
...
Hooks.on("updateCombat", (S, s, e) => {
  (s == null ? void 0 : s.round) != null && zs(S), setTimeout(Us, 0);
});
...
//# sourceMappingURL=vacuum-of-the-void-D-XA-P4o.mjs.map
