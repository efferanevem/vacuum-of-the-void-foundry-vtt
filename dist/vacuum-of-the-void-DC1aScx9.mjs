...
const B = class B extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(s) {
    ...
    const b = this.actor.system.weapons ?? {}, v = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], I = (b.slotOrder ?? "").trim(), O = I ? I.split(/\s*,\s*/).filter((f) => v.includes(f)) : [], q = (Re = this.actor.system.gear) == null ? void 0 : Re.microchips, C = q && typeof q == "object" && !Array.isArray(q) ? q : {}, T = (((Ke = C.slot1) == null ? void 0 : Ke.itemId) ?? "").trim(), S = (((Fe = C.slot2) == null ? void 0 : Fe.itemId) ?? "").trim(), A = (((Ee = C.slot3) == null ? void 0 : Ee.itemId) ?? "").trim(), H = this.actor.items.contents.filter((f) => f.type !== "Kepesseg" && f.type !== "ability").slice().sort((f, D) => {
    ...
    e.actionsTable = H;
    const U = (((tt = this.actor.items) == null ? void 0 : tt.contents) ?? []).filter((f) => f.type === "Fegyver"), Z = U.map((f) => ({ id: f.id, name: f.name, img: f.img }));
    e.weaponItems = Z;
    const W = "— Nincs fegyver —", ee = O.filter((f) => {
    ...
    e.weaponSlots = ee.map((f, D) => {
      ...
      return {
        ...
        itemId: L,
        slotNum: D + 1,
        displayName: (M == null ? void 0 : M.name) || _.name || W,
    ...
    const ue = (((nt = this.actor.items) == null ? void 0 : nt.contents) ?? []).filter((f) => f.type === "MikroChip"), V = ue.map((f) => ({ id: f.id, name: f.name, img: f.img })), ye = "— Nincs Mikro-Chip —", ge = ["slot1", "slot2", "slot3"], de = (f) => {
      var _, L, M;
      const D = [];
    ...
    e.microchipItemsSlot1 = V.filter((f) => !de(1).includes(f.id)), e.microchipItemsSlot2 = V.filter((f) => !de(2).includes(f.id)), e.microchipItemsSlot3 = V.filter((f) => !de(3).includes(f.id)), e.microchipSlots = ge.map((f) => {
      const D = C[f] ?? {}, _ = (D.itemId ?? "").trim(), L = ue.find((M) => M.id === _);
      return {
        slotKey: f,
        itemId: _,
      ...
      const D = `slot${f}`, _ = C[D] ?? {}, L = (_.itemId ?? "").trim(), M = V.find((J) => J.id === L);
    ...
    const te = this.actor.system.abilities ?? {}, k = (((at = this.actor.items) == null ? void 0 : at.contents) ?? []).map((f) => {
      var D, _, L;
      return {
        id: f.id,
        name: f.name,
    ...
    }), F = new Map(k.map((f) => [f.id, f])), x = "— Nincs képesség —", R = (f) => {
      const D = te[f] ?? {}, _ = (D.itemId ?? "").trim(), L = _ ? F.get(_) : null, M = (L == null ? void 0 : L.description) ?? "", J = (L == null ? void 0 : L.kp) ?? 0;
      return {
        itemId: _,
    ...
    e.abilityFajiSlot = R("faji"), e.abilityHatterSlot = R("hatter");
    const he = (f) => {
      const D = te[f];
  ...
  _attachFrameListeners(s) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (i) => {
      ...
      if (!a) return;
      const { openRollSheetForWeapon: n } = await import("./roll-sheet-Ce3PPvs3.mjs");
      n(this.actor, a);
    ...
    d && o({ currentTarget: d }), this._writeEffectiveGivenSpeed(s), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(s), this._writeEffectiveGivenProtection(e), requestAnimationFrame(() => {
      ...
      var p;
      i.preventDefault();
      const r = i.currentTarget, a = r.dataset.skill, n = ((p = r.textContent) == null ? void 0 : p.trim()) || a, { openRollSheetForSkill: l } = await import("./roll-sheet-Ce3PPvs3.mjs");
      l(this.actor, a, n);
...
z(B, "PARTS", foundry.utils.mergeObject(
  ...
  K(B, B, "DEFAULT_OPTIONS"),
  {
    ...
    minWidth: 1e3,
    height: 800,
    form: foundry.utils.mergeObject(((yt = K(B, B, "DEFAULT_OPTIONS")) == null ? void 0 : yt.form) ?? {}, { submitOnChange: !1 }),
    window: foundry.utils.mergeObject(((gt = K(B, B, "DEFAULT_OPTIONS")) == null ? void 0 : gt.window) ?? {}, {
      resizable: !0,
      contentClasses: [
        ...((bt = (ht = K(B, B, "DEFAULT_OPTIONS")) == null ? void 0 : ht.window) == null ? void 0 : bt.contentClasses) ?? [],
        "votv-scroll"
      ]
    })
  },
...
const X = class X extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(s) {
    ...
    const t = this.actor, o = ((Ie = t == null ? void 0 : t.items) == null ? void 0 : Ie.contents) ?? [], d = ((Se = t == null ? void 0 : t.system) == null ? void 0 : Se.weapons) ?? {}, c = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], u = (d.slotOrder ?? "").trim(), y = u ? u.split(/\s*,\s*/).filter((k) => c.includes(k)) : [], m = o.filter((k) => k.type === "Fegyver"), g = m.map((k) => ({ id: k.id, name: k.name, img: k.img })), h = "— Nincs fegyver —", i = y.length > 0 ? y : c, r = c.filter((k) => !i.includes(k)), a = [...i, ...r], n = a.filter((k) => {
      ...
      return F && m.some((R) => R.id === F);
    });
    e.weaponSlots = n.map((k) => {
      const F = d[k] ?? {}, x = (F.itemId ?? "").trim(), R = g.find((he) => he.id === x);
      return {
        slotKey: k,
        itemId: x,
        displayName: (R == null ? void 0 : R.name) || F.name || h,
    ...
    e.weaponsTable = m.map((k) => {
      ...
      const he = !!x, G = (k == null ? void 0 : k.system) ?? {}, Pe = typeof G.range == "string" ? (G.range || "").trim() : "", me = G.type ?? "", be = (l[me] ?? me) || "—", pe = G.size ?? "", _e = (p[pe] ?? pe) || "", De = [be, _e].filter(Boolean).join(", ") || be || "—", Me = me === "projectile" || pe === "thrown", je = G.quantity != null ? String(G.quantity).trim() : "1", Re = String(G.bonus ?? R.bonus ?? "").trim() || "0", Ke = (G.damage ?? R.damage ?? "").trim() || "";
      return {
        slotKey: x ?? "",
        itemId: F,
        actorId: t.id,
    ...
    e.armorTable = b.map((k) => {
      var R;
      const F = (k == null ? void 0 : k.system) ?? {}, x = ((R = k.system) == null ? void 0 : R.equipped) === !0;
      return {
        itemId: k.id,
        actorId: t.id,
    ...
    const v = ((Te = (le = t == null ? void 0 : t.system) == null ? void 0 : le.gear) == null ? void 0 : Te.microchips) ?? {}, I = ["slot1", "slot2", "slot3"], O = o.filter((k) => k.type === "MikroChip");
    e.microchipsTable = O.map((k) => {
      ...
      const R = ((be = k == null ? void 0 : k.system) == null ? void 0 : be.abilityType) ?? "", he = R === "active" ? "Aktív" : R === "passive" ? "Passzív" : R || "—", G = (((pe = k == null ? void 0 : k.system) == null ? void 0 : pe.description) ?? "").trim(), Pe = G ? G.length > 60 ? G.slice(0, 57) + "…" : G : "—";
      return {
        itemId: F,
        actorId: t.id,
        name: (k == null ? void 0 : k.name) ?? "—",
    ...
    e.itemsTable = q.map((k) => {
      const F = (k == null ? void 0 : k.system) ?? {}, x = (F.description ?? "").trim(), R = x ? x.length > 60 ? x.slice(0, 57) + "…" : x : "—";
      return {
        itemId: k.id,
        actorId: t.id,
        name: (k == null ? void 0 : k.name) ?? "—",
    ...
    e.egyebList = C.map((k) => {
      const x = (((k == null ? void 0 : k.system) ?? {}).description ?? "").trim(), R = x ? x.length > 80 ? x.slice(0, 77) + "…" : x : "";
      return {
        itemId: k.id,
        actorId: t.id,
        name: (k == null ? void 0 : k.name) ?? "—",
    ...
    const S = ((te = t == null ? void 0 : t.system) == null ? void 0 : te.abilities) ?? {}, A = o.filter((k) => k.type === "Kepesseg" || k.type === "ability").map((k) => {
      var F, x;
      return {
        id: k.id,
        name: k.name,
  ...
  _attachFrameListeners(s) {
    ...
    t.on("click", ".karakter-inventory-item-name", (i) => {
      ...
      if (!a || !this.actor) return;
      const { openRollSheetForSkill: l } = await import("./roll-sheet-Ce3PPvs3.mjs");
      l(this.actor, a, n);
    }), t.on("click", ".karakter-ability-chat", async (i) => {
      var a, n;
      i.preventDefault();
  ...
  async _onDropItem(s, e) {
    var g, h, i, r, a, n, l, p, b, v, I, O, q, C, T;
    ...
    if (d.type === "Fegyver" && m) {
      const S = ((I = c.system) == null ? void 0 : I.weapons) ?? {}, A = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], N = (S.slotOrder ?? "").trim(), H = N ? N.split(/\s*,\s*/).filter((W) => A.includes(W)) : [];
      let U = A.find((W) => {
        var ee;
        return !(((ee = S[W]) == null ? void 0 : ee.itemId) ?? "").trim();
      });
      U || (U = A.find((W) => !H.includes(W)) ?? "slot1");
      const Z = H.includes(U) ? H : [...H, U].filter((W) => A.includes(W));
      await c.update({
...
z(X, "PARTS", foundry.utils.mergeObject(
  ...
  K(X, X, "DEFAULT_OPTIONS"),
  {
    ...
    minWidth: 800,
    height: 720,
    form: foundry.utils.mergeObject(((kt = K(X, X, "DEFAULT_OPTIONS")) == null ? void 0 : kt.form) ?? {}, { submitOnChange: !1 }),
    window: foundry.utils.mergeObject(((vt = K(X, X, "DEFAULT_OPTIONS")) == null ? void 0 : vt.window) ?? {}, {
      resizable: !0,
      contentClasses: [
...
z(ie, "PARTS", foundry.utils.mergeObject(K(ie, ie, "PARTS") ?? {}, {
  ...
  minWidth: 550,
  height: 400,
  form: foundry.utils.mergeObject(((St = K(ie, ie, "DEFAULT_OPTIONS")) == null ? void 0 : St.form) ?? {}, { submitOnChange: !1 })
}));
let Je = ie;
var Tt;
...
z(re, "PARTS", foundry.utils.mergeObject(K(re, re, "PARTS") ?? {}, {
  ...
  minWidth: 550,
  height: 400,
  form: foundry.utils.mergeObject(((Tt = K(re, re, "DEFAULT_OPTIONS")) == null ? void 0 : Tt.form) ?? {}, { submitOnChange: !1 })
}));
let Qe = re;
var _t;
...
z(ne, "PARTS", foundry.utils.mergeObject(K(ne, ne, "PARTS") ?? {}, {
  ...
  minWidth: 500,
  height: 420,
  form: foundry.utils.mergeObject(((_t = K(ne, ne, "DEFAULT_OPTIONS")) == null ? void 0 : _t.form) ?? {}, { submitOnChange: !1 })
}));
let Xe = ne;
var Dt;
...
z(ae, "PARTS", foundry.utils.mergeObject(K(ae, ae, "PARTS") ?? {}, {
  ...
  minWidth: 500,
  height: 420,
  form: foundry.utils.mergeObject(((Dt = K(ae, ae, "DEFAULT_OPTIONS")) == null ? void 0 : Dt.form) ?? {}, { submitOnChange: !0 })
}));
let Ze = ae;
var Ot;
...
z(oe, "PARTS", foundry.utils.mergeObject(K(oe, oe, "PARTS") ?? {}, {
  ...
  minWidth: 400,
  height: 360,
  form: foundry.utils.mergeObject(((Ot = K(oe, oe, "DEFAULT_OPTIONS")) == null ? void 0 : Ot.form) ?? {}, { submitOnChange: !1 })
}));
let Ve = oe;
var Ct;
...
z(ce, "PARTS", foundry.utils.mergeObject(K(ce, ce, "PARTS") ?? {}, {
  ...
  minWidth: 400,
  height: 360,
  form: foundry.utils.mergeObject(((Ct = K(ce, ce, "DEFAULT_OPTIONS")) == null ? void 0 : Ct.form) ?? {}, { submitOnChange: !1 })
}));
let et = ce;
  ...
  if (y) {
    ...
    if (c && u && (u.actorId || (h = E.speaker) != null && h.actor)) {
      ...
      if (u.isHit && !y.querySelector(".votv-weapon-damage-chat")) {
        ...
        p.textContent = b != null && v ? `Sebzés (Találati Hely ${b} - ${v})` : "Sebzés", p.dataset.actorId = String(u.actorId ?? ((i = E.speaker) == null ? void 0 : i.actor) ?? ""), p.dataset.itemId = String(u.itemId ?? ""), p.dataset.slotKey = String(u.slotKey ?? ""), p.dataset.targetName = String(u.targetName ?? ""), p.dataset.hitLocationName = String(u.hitLocationName ?? ""), a.appendChild(p), p.addEventListener("click", async (I) => {
          var we, Ie, Se, le, Te, ue, V, ye, ge, de, te, k, F, x;
          I.preventDefault(), I.stopPropagation();
          const O = I.currentTarget, q = (O.dataset.actorId ?? "").trim(), C = (O.dataset.itemId ?? "").trim(), T = (O.dataset.slotKey ?? "").trim(), S = (O.dataset.targetName ?? "").trim(), A = (O.dataset.hitLocationName ?? "").trim(), N = q && ((we = game.actors) == null ? void 0 : we.get(q)) || ((Ie = E.speaker) == null ? void 0 : Ie.actor) && ((Se = game.actors) == null ? void 0 : Se.get(E.speaker.actor)) || null;
          if (!N) return;
          let H = C ? ((Te = (le = N.items) == null ? void 0 : le.get) == null ? void 0 : Te.call(le, C)) ?? ((ye = (V = (ue = N.items) == null ? void 0 : ue.contents) == null ? void 0 : V.find) == null ? void 0 : ye.call(V, (R) => R.id === C)) : null;
          if (!H || H.type !== "Fegyver") return;
          let U = (((ge = H.system) == null ? void 0 : ge.damage) ?? "").trim();
          if (!U && T && (U = (((((de = N.system) == null ? void 0 : de.weapons) ?? {})[T] ?? {}).damage ?? "").trim()), !U) {
            (x = (te = ui.notifications) == null ? void 0 : te.warn) == null || x.call(
              te,
              ((F = (k = game.i18n) == null ? void 0 : k.localize) == null ? void 0 : F.call(k, "votv.weapon.no-damage")) ?? "A fegyvernek nincs sebzés formulája."
            );
            return;
          }
          const Z = new Roll(U);
          ...
          let ee = S ? `${H.name} – sebzés – ${S}` : `${H.name} – sebzés`;
          A && (ee += ` (${A})`), await Z.toMessage({
...
//# sourceMappingURL=vacuum-of-the-void-DC1aScx9.mjs.map
