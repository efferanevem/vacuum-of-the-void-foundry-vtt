...
const B = class B extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  _writeTotalSpeed(i = null) {
    ...
    const m = ((v = (h = this.actor) == null ? void 0 : h.system) == null ? void 0 : v.resources) ?? {}, p = ((S = (I = m.hitLocations) == null ? void 0 : I.legs) == null ? void 0 : S.value) ?? 0, y = ((O = m.currentHealth) == null ? void 0 : O.legs) ?? 0;
  ...
  async _prepareContext(i) {
    ...
    for (const g of l) {
      const D = ((Qe = d[g]) == null ? void 0 : Qe.value) ?? 0, _ = a[g] ?? 0;
    ...
    const S = this.actor.system.weapons ?? {}, O = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], L = (S.slotOrder ?? "").trim(), N = L ? L.split(/\s*,\s*/).filter((g) => O.includes(g)) : [], C = (Le = this.actor.system.gear) == null ? void 0 : Le.microchips, E = C && typeof C == "object" && !Array.isArray(C) ? C : {}, F = (((Ne = E.slot1) == null ? void 0 : Ne.itemId) ?? "").trim(), z = (((mt = E.slot2) == null ? void 0 : mt.itemId) ?? "").trim(), G = (((pt = E.slot3) == null ? void 0 : pt.itemId) ?? "").trim(), V = this.actor.items.contents.filter((g) => g.type !== "Kepesseg" && g.type !== "ability").slice().sort((g, D) => {
      const _ = typeof g.sort == "number" ? g.sort : 0, A = typeof D.sort == "number" ? D.sort : 0;
      return _ - A;
    }).map((g) => {
      ...
      for (let xe = 0; xe < N.length; xe++) {
        const tt = N[xe];
        if ((((ve = S[tt]) == null ? void 0 : ve.itemId) ?? "") === g.id) {
          A = xe + 1, M = ((Fe = S[tt]) == null ? void 0 : Fe.bonus) ?? "", ee = tt;
          break;
        }
      }
      let X = null;
      _ && (F === g.id ? X = "slot1" : z === g.id ? X = "slot2" : G === g.id && (X = "slot3"));
      const Ze = D && ((Pe = g.system) != null && Pe.damage) ? g.system.damage : "", le = D && typeof ((Te = g.system) == null ? void 0 : Te.range) == "string" ? (g.system.range || "").trim() : "", Ae = ee ? Number((_e = S[ee]) == null ? void 0 : _e.bonus) || 0 : "";
      return {
        id: g.id,
        name: g.name,
        img: g.img,
        ...
        rangeStr: le,
        slotBonusNum: Ae,
        quantity: Number(((et = g.system) == null ? void 0 : et.quantity) ?? 1) || 1
      };
    });
    e.actionsTable = V;
    const Z = (((ft = this.actor.items) == null ? void 0 : ft.contents) ?? []).filter((g) => g.type === "Fegyver"), Y = Z.map((g) => ({ id: g.id, name: g.name, img: g.img }));
    e.weaponItems = Y;
    const R = "— Nincs fegyver —", W = N.filter((g) => {
      const _ = ((S[g] ?? {}).itemId ?? "").trim();
      return _ ? !!Z.find((M) => M.id === _) : !1;
    });
    e.weaponSlots = W.map((g, D) => {
      const _ = S[g] ?? {}, A = (_.itemId ?? "").trim(), M = Y.find((ee) => ee.id === A);
      return {
        slotKey: g,
        itemId: A,
        slotNum: D + 1,
    ...
    e.weaponsTable = Z.map((g) => {
      ...
      const M = !!_, ee = ((Bt = g.system) == null ? void 0 : Bt.equipped) !== void 0 && ((Gt = g.system) == null ? void 0 : Gt.equipped) !== null ? !!g.system.equipped : M, X = (g == null ? void 0 : g.system) ?? {}, Ze = typeof X.range == "string" ? (X.range || "").trim() : "", le = X.type ?? "", Ae = (oe[le] ?? le) || "—", ve = X.size ?? "", Fe = (ge[ve] ?? ve) || "", Pe = [Ae, Fe].filter(Boolean).join(", ") || Ae || "—", Te = le === "projectile" || ve === "thrown", _e = X.quantity, et = _e != null ? String(_e).trim() : "1", xe = String(A.bonus ?? "").trim(), tt = String(X.bonus ?? "").trim(), bi = xe || tt || "0";
      return {
        slotKey: _ ?? "",
        itemId: D,
        actorId: this.actor.id,
        name: (g == null ? void 0 : g.name) ?? R,
    ...
    const re = (((yt = this.actor.items) == null ? void 0 : yt.contents) ?? []).filter((g) => g.type === "Pancel");
    e.armorTable = re.map((g) => {
      var A, M;
      const D = (g == null ? void 0 : g.system) ?? {}, _ = ((A = g.system) == null ? void 0 : A.equipped) !== void 0 && ((M = g.system) == null ? void 0 : M.equipped) !== null ? !!g.system.equipped : !1;
      return {
        itemId: g.id,
        actorId: this.actor.id,
    ...
    const Re = (((Xe = this.actor.items) == null ? void 0 : Xe.contents) ?? []).filter((g) => g.type === "Targy");
    e.itemsTable = Re.map((g) => {
      const D = (g == null ? void 0 : g.system) ?? {}, _ = (D.description ?? "").trim(), A = _ ? _.length > 60 ? _.slice(0, 57) + "…" : _ : "—", M = D.quantity != null ? String(D.quantity).trim() : "1";
      return {
        itemId: g.id,
        actorId: this.actor.id,
        name: (g == null ? void 0 : g.name) ?? "—",
    ...
    const ie = ((($e = this.actor.items) == null ? void 0 : $e.contents) ?? []).filter((g) => g.type === "Egyeb");
    ...
    const se = (((Lt = this.actor.items) == null ? void 0 : Lt.contents) ?? []).filter((g) => g.type === "MikroChip"), ne = se.map((g) => ({ id: g.id, name: g.name, img: g.img })), Me = "— Nincs Mikro-Chip —", Ke = ["slot1", "slot2", "slot3"], je = (g) => {
      var _, A, M;
      const D = [];
      return g !== 1 && D.push((((_ = E.slot1) == null ? void 0 : _.itemId) ?? "").trim()), g !== 2 && D.push((((A = E.slot2) == null ? void 0 : A.itemId) ?? "").trim()), g !== 3 && D.push((((M = E.slot3) == null ? void 0 : M.itemId) ?? "").trim()), D.filter(Boolean);
    };
    e.microchipItemsSlot1 = ne.filter((g) => !je(1).includes(g.id)), e.microchipItemsSlot2 = ne.filter((g) => !je(2).includes(g.id)), e.microchipItemsSlot3 = ne.filter((g) => !je(3).includes(g.id)), e.microchipSlots = Ke.map((g) => {
      const D = E[g] ?? {}, _ = (D.itemId ?? "").trim(), A = se.find((M) => M.id === _);
      return {
        slotKey: g,
        itemId: _,
      ...
      const D = `slot${g}`, _ = E[D] ?? {}, A = (_.itemId ?? "").trim(), M = ne.find((ee) => ee.id === A);
      e[`microchip${g}ItemId`] = A, e[`microchip${g}DisplayName`] = (M == null ? void 0 : M.name) || _.name || Me, e[`microchip${g}Img`] = (M == null ? void 0 : M.img) || "";
      ...
      const A = !!_, M = ((Fe = g == null ? void 0 : g.system) == null ? void 0 : Fe.abilityType) ?? "", ee = M === "active" ? "Aktív" : M === "passive" ? "Passzív" : M || "—", X = M === "active", Ze = Number((Pe = g == null ? void 0 : g.system) == null ? void 0 : Pe.replaceCost) >= 0 ? Number(g.system.replaceCost) : 0, le = (((Te = g == null ? void 0 : g.system) == null ? void 0 : Te.description) ?? "").trim(), Ae = le ? le.length > 60 ? le.slice(0, 57) + "…" : le : "—";
      return {
        itemId: D,
        actorId: this.actor.id,
        name: (g == null ? void 0 : g.name) ?? "—",
    ...
    const Ee = this.actor.system.abilities ?? {}, nt = (((Et = this.actor.items) == null ? void 0 : Et.contents) ?? []).map((g) => {
      var D, _, A;
      return {
        id: g.id,
        name: g.name,
    ...
    }), ze = new Map(nt.map((g) => [g.id, g])), at = "— Nincs képesség —", He = (g) => {
      const D = Ee[g] ?? {}, _ = (D.itemId ?? "").trim(), A = _ ? ze.get(_) : null, M = (A == null ? void 0 : A.description) ?? "", ee = (A == null ? void 0 : A.kp) ?? 0;
      return {
        itemId: _,
    ...
    const rt = (($t = e.system) == null ? void 0 : $t.resources) ?? {}, ot = ["kpDot1", "kpDot2", "kpDot3", "kpDot4", "kpDot5", "kpDot6"], Oe = (Rt = (xt = e.system) == null ? void 0 : xt.combat) == null ? void 0 : Rt.initiativeResult, he = typeof Oe == "number" ? Oe : -1, ct = [
      ...
      { min: 19, max: 24, kp: 5 },
      { min: 25, max: 999, kp: 6 }
    ], Ge = (Kt = (Mt = e.system) == null ? void 0 : Mt.combat) == null ? void 0 : Kt.initiativeRanges, Ce = Array.isArray(Ge) ? Ge.map((g) => ({ min: Number(g.min) ?? 0, max: Number(g.max) ?? 0, kp: Math.min(10, Math.max(0, Number(g.kp) ?? 0)) })) : [], we = (Ce.length > 0 ? Ce : ct).slice().sort((g, D) => g.min - D.min), qe = Ce.map((g) => g.min), lt = qe.length ? Math.min(...qe) : 0, ut = qe.length ? Math.max(...qe) : 0;
    e.initiativeRanges = Ce.map((g) => ({ ...g, isFirst: g.min === lt, isLast: g.min === ut }));
    let Ve = 0;
    if (typeof he == "number" && he >= 0 && we.length > 0) {
      const g = we[0], D = we[we.length - 1];
      let _ = null;
      he <= g.max ? _ = g : he >= D.min ? _ = D : _ = we.find((A) => he >= A.min && he <= A.max), _ && (Ve = _.kp);
    }
    e.kpDots = ot.map((g, D) => {
      const _ = D + 1, A = !!Number(rt[g]), M = _ <= Ve;
      return { index: _, used: A, usable: M };
    });
    const dt = !!((zt = (jt = game.combat) == null ? void 0 : jt.combatants) != null && zt.some((g) => {
      var D;
  ...
  _attachFrameListeners(i) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (s) => {
      ...
      if (!o) return;
      const { openRollSheetForWeapon: r } = await import("./roll-sheet-DXRjJEtp.mjs");
      r(this.actor, o);
      ...
      s.preventDefault(), s.stopPropagation();
      const { openRollSheetForSkill: c } = await import("./roll-sheet-DXRjJEtp.mjs");
    ...
    t.on("input change", 'input[name="system.combat.defense"], input[name="system.combat.defenseBonus"], input[name="system.combat.speed"], input[name="system.combat.speedBonus"]', a), this._writeEffectiveGivenSpeed(i), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(i), this._writeEffectiveGivenProtection(e), this._writeTotalDefense(i), this._writeTotalDefense(e), this._writeTotalSpeed(i), this._writeTotalSpeed(e), requestAnimationFrame(() => {
      ...
      var h;
      s.preventDefault();
      const c = s.currentTarget, o = c.dataset.skill, r = ((h = c.textContent) == null ? void 0 : h.trim()) || o, { openRollSheetForSkill: f } = await import("./roll-sheet-DXRjJEtp.mjs");
      f(this.actor, o, r);
  ...
  async _rollSkill(i, e) {
    ...
    const { resultType: y, label: b } = wt(p), u = game.settings.get("core", "rollMode") ?? CONST.DICE_ROLL_MODES.PUBLIC;
    return p.toMessage({
      speaker: ChatMessage.getSpeaker({ actor: this.actor }),
      flavor: e,
...
const ae = class ae extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(i) {
    ...
    e.weaponSlots = o.map((k) => {
      const P = d[k] ?? {}, x = (P.itemId ?? "").trim(), j = y.find((ce) => ce.id === x);
      return {
        slotKey: k,
        itemId: x,
  ...
  _attachFrameListeners(i) {
    ...
    t.on("click", ".karakter-inventory-item-name", (u) => {
      ...
      if (!c || !this.actor) return;
      const { openRollSheetForWeapon: o } = await import("./roll-sheet-DXRjJEtp.mjs");
      o(this.actor, c);
      ...
      if (u.preventDefault(), u.stopPropagation(), !this.actor) return;
      const { openRollSheetForSkill: s } = await import("./roll-sheet-DXRjJEtp.mjs");
      ...
      if (!c || !this.actor) return;
      const { openRollSheetForSkill: r } = await import("./roll-sheet-DXRjJEtp.mjs");
      r(this.actor, c, o);
  ...
  async _rollWeapon(i) {
    ...
    const m = l && l.name || n.name || "Fegyver", p = Number(n.bonus || 0) || 0, y = ((N = l == null ? void 0 : l.system) == null ? void 0 : N.type) || "kinetic", b = (((C = l == null ? void 0 : l.system) == null ? void 0 : C.skillKey) ?? "").trim(), u = b && d[b] !== void 0 ? b : y === "explosive" ? "grenadeUse" : "firearms", s = Number(d[u] || 0) || 0, c = p + s, r = `3d6${c !== 0 ? c > 0 ? `+${c}` : `${c}` : ""}`, f = new Roll(r);
  ...
  async _onDropItem(i, e) {
    var y, b, u, s, c, o, r, f, h, v, I, S, O, L, N, C, E;
...
Hooks.on("ready", () => {
  ...
  document.body.addEventListener("click", async (e) => {
    ...
    if (y || (y = (((Z = p == null ? void 0 : p.system) == null ? void 0 : Z.damage) ?? "").trim()), !y && a.slotKey && (y = (((((Y = m.system) == null ? void 0 : Y.weapons) ?? {})[a.slotKey] ?? {}).damage ?? "").trim()), !y) {
    ...
    for (const l of e) {
      const m = st.getTotalSpeedRaw(l), p = (n = l.effects) == null ? void 0 : n.find((y) => y.getFlag("vacuum-of-the-void", "immobilizedFromSpeed") === !0);
      if (m <= 0 && !p) {
...
Hooks.on("preCreateToken", (w, i, e) => {
  ...
  if (d) {
    ...
    if (d.type === "Npc") {
      const y = yi((p = (m = d.system) == null ? void 0 : m.identity) == null ? void 0 : p.size);
      w.updateSource({
        ...t ? { actorId: t } : {},
        "bar1.attribute": "",
        "bar2.attribute": "resources.health",
...
//# sourceMappingURL=vacuum-of-the-void-Dlwb7Uqz.mjs.map
