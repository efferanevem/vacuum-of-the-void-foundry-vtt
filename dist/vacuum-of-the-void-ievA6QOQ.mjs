...
const { NumberField: de, StringField: Re } = foundry.data.fields;
...
const { NumberField: bt, StringField: Ue } = foundry.data.fields;
...
const E = class E extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(s) {
    var qe, Te, De, Oe, _e, Le, Ae, Fe, Ne, Ee, Ce, $e, xe, Pe, je, Ke;
    ...
    for (const u of d) {
      const v = ((Oe = n[u]) == null ? void 0 : Oe.value) ?? 0, q = o[u] ?? 0;
    ...
    const c = this.actor.system.weapons ?? {}, m = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], f = (c.slotOrder ?? "").trim(), k = f ? f.split(/\s*,\s*/).filter((u) => m.includes(u)) : [], S = (Ae = this.actor.system.gear) == null ? void 0 : Ae.microchips, O = S && typeof S == "object" && !Array.isArray(S) ? S : {}, F = (((Fe = O.slot1) == null ? void 0 : Fe.itemId) ?? "").trim(), A = (((Ne = O.slot2) == null ? void 0 : Ne.itemId) ?? "").trim(), _ = (((Ee = O.slot3) == null ? void 0 : Ee.itemId) ?? "").trim(), C = this.actor.items.contents.filter((u) => u.type !== "Kepesseg" && u.type !== "ability").slice().sort((u, v) => {
    ...
    e.actionsTable = C;
    const j = (((Ce = this.actor.items) == null ? void 0 : Ce.contents) ?? []).filter((u) => u.type === "Fegyver"), R = j.map((u) => ({ id: u.id, name: u.name, img: u.img }));
    e.weaponItems = R;
    const ge = "— Nincs fegyver —", Ve = k.filter((u) => {
    ...
    e.weaponSlots = Ve.map((u, v) => {
      ...
      return {
        ...
        itemId: T,
        slotNum: v + 1,
        displayName: (L == null ? void 0 : L.name) || q.name || ge,
    ...
    e.weaponsTable = j.map((u) => {
      ...
      const L = !!q, x = ((Z = u.system) == null ? void 0 : Z.equipped) !== void 0 && ((Me = u.system) == null ? void 0 : Me.equipped) !== null ? !!u.system.equipped : L, K = (u == null ? void 0 : u.system) ?? {}, X = typeof K.range == "string" ? (K.range || "").trim() : "", Y = K.type ?? "", W = (et[Y] ?? Y) || "—", V = K.size ?? "", re = (tt[V] ?? V) || "", ae = [W, re].filter(Boolean).join(", ") || W || "—", ee = W === "Lövedékes", te = K.quantity, se = te != null ? String(te).trim() : "1";
      return {
        slotKey: q ?? "",
        itemId: v,
        actorId: this.actor.id,
        name: (u == null ? void 0 : u.name) ?? ge,
    ...
    const st = ((($e = this.actor.items) == null ? void 0 : $e.contents) ?? []).filter((u) => u.type === "Pancél");
    ...
    }), e.generalItems = Array.isArray((Pe = (xe = this.actor.system) == null ? void 0 : xe.gear) == null ? void 0 : Pe.generalItems) ? this.actor.system.gear.generalItems : [];
    const he = (((je = this.actor.items) == null ? void 0 : je.contents) ?? []).filter((u) => u.type === "microchip"), ie = he.map((u) => ({ id: u.id, name: u.name, img: u.img }));
    e.microchipsTable = ["1", "2", "3"].map((u) => {
      var X, Y;
      const v = `slot${u}`, q = O[v] ?? {}, T = (q.itemId ?? "").trim(), L = he.find((W) => W.id === T), x = ((X = L == null ? void 0 : L.system) == null ? void 0 : X.abilityType) ?? "", K = x === "active" ? "Aktív" : x === "passive" ? "Passzív" : x || "—";
      return {
        slotKey: v,
        itemId: T,
        name: (L == null ? void 0 : L.name) ?? q.name ?? be,
        typeLabel: K,
        description: (((Y = L == null ? void 0 : L.system) == null ? void 0 : Y.description) ?? "").trim() || "—"
      };
    });
    const be = "— Nincs Mikro-Chip —", le = (u) => {
      const v = [];
    ...
    e.microchipItemsSlot1 = ie.filter((u) => !le(1).includes(u.id)), e.microchipItemsSlot2 = ie.filter((u) => !le(2).includes(u.id)), e.microchipItemsSlot3 = ie.filter((u) => !le(3).includes(u.id)), ["1", "2", "3"].forEach((u) => {
      ...
      e[`microchip${u}ItemId`] = T, e[`microchip${u}DisplayName`] = (L == null ? void 0 : L.name) || q.name || be, e[`microchip${u}Img`] = (L == null ? void 0 : L.img) || "", e[`microchip${u}Active`] = q.active === !0;
    });
    const we = this.actor.system.abilities ?? {}, it = (((Ke = this.actor.items) == null ? void 0 : Ke.contents) ?? []).map((u) => {
      var v, q, T;
      return {
        id: u.id,
        name: u.name,
    ...
    }), ke = new Map(it.map((u) => [u.id, u])), rt = "— Nincs képesség —", ve = (u) => {
      const v = we[u] ?? {}, q = (v.itemId ?? "").trim(), T = q ? ke.get(q) : null, L = (T == null ? void 0 : T.description) ?? "", x = (T == null ? void 0 : T.kp) ?? 0;
      return {
        itemId: q,
  ...
  _attachFrameListeners(s) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (r) => {
      ...
      if (!l) return;
      const { openRollSheetForWeapon: c } = await import("./roll-sheet-B3qO_ChY.mjs");
      c(this.actor, l);
    ...
    n && i({ currentTarget: n }), this._writeEffectiveGivenSpeed(s), this._writeEffectiveGivenSpeed(e), requestAnimationFrame(() => this._writeEffectiveGivenSpeed()), setTimeout(() => this._writeEffectiveGivenSpeed(), 100), t.on("click", ".karakter-portrait-img", (r) => {
      ...
      var f;
      r.preventDefault();
      const a = r.currentTarget, l = a.dataset.skill, c = ((f = a.textContent) == null ? void 0 : f.trim()) || l, { openRollSheetForSkill: m } = await import("./roll-sheet-B3qO_ChY.mjs");
      m(this.actor, l, c);
...
Hooks.once("init", () => {
  ...
  }, (s = CONFIG.Actor).dataModels ?? (s.dataModels = {}), CONFIG.Actor.dataModels.Karakter = mt, (e = CONFIG.Item).dataModels ?? (e.dataModels = {}), CONFIG.Item.dataModels.Fegyver = ft, CONFIG.Item.dataModels.Pajzs = gt, CONFIG.Item.dataModels.microchip = ht, CONFIG.Item.dataModels.Kepesseg = He, CONFIG.Item.dataModels.ability = He, (t = CONFIG.Actor).trackableAttributes ?? (t.trackableAttributes = {}), CONFIG.Actor.trackableAttributes.Karakter = {
...
//# sourceMappingURL=vacuum-of-the-void-ievA6QOQ.mjs.map
