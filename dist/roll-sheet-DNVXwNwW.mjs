import { V as q, c as Y } from "./vacuum-of-the-void-BHExYGt0.mjs";
class U extends Application {
  ...
  _bindRollSheetListeners() {
    ...
    v && v.addEventListener("input", () => t._updateFormulaPreview(e)), v && v.addEventListener("change", () => t._updateFormulaPreview(e)), t._updateFormulaPreview(e), e._votvRollBound = !0, e.addEventListener("click", (s) => {
      ...
      (I = (f = s.target) == null ? void 0 : f.closest) != null && I.call(f, ".roll-sheet-do-roll") && (s.preventDefault(), s.stopPropagation(), t._doRoll(e));
    });
  }
  ...
  async _doRoll(t) {
    ...
    const a = this.actor;
    if (!a) return;
    const e = t.querySelector("[name='moraleDice']"), o = t.querySelector("[name='advantage']"), u = t.querySelector("[name='constantBonus']"), d = Math.min(3, Math.max(0, parseInt((e == null ? void 0 : e.value) ?? "0", 10) || 0)), m = Math.max(0, Number(((B = (I = a.system) == null ? void 0 : I.resources) == null ? void 0 : B.morale) ?? 0) || 0), i = Math.min(d, m), r = Math.min(3, Math.max(-3, parseInt((o == null ? void 0 : o.value) ?? "0", 10) || 0)), c = parseInt((u == null ? void 0 : u.value) ?? "0", 10) || 0, h = this._baseModifier, v = this._injuryModifier, y = this._injuryPenalty || 0, l = h + v + y + c, p = [
      "3d6",
    ...
    if (this._isWeaponAttack && ((C = (R = game.user) == null ? void 0 : R.targets) != null && C.size)) {
      const L = Array.from(game.user.targets)[0], S = (L == null ? void 0 : L.actor) ?? null;
      if (S) {
        const k = ((E = S.system) == null ? void 0 : E.combat) ?? {}, z = Number(k.defense ?? 0) || 0, T = Number(k.defenseBonus ?? 0) || 0, W = Number(k.givenProtection ?? 0) || 0;
        let V = 0;
        if (S.type === "Karakter") {
          const P = (((A = S.items) == null ? void 0 : A.contents) ?? []).filter(
            (N) => {
              var x;
          ...
          for (const N of P) {
            ...
            Number.isFinite(K) && (V += K);
          }
        }
        const F = z + T + W + V, O = (Number(g.total ?? 0) || 0) >= F, H = S.name ?? L.name ?? "Célpont";
        let w = null, j = null;
        if (O && S.type === "Karakter") {
          const P = new Roll("1d8");
          await P.evaluate({ async: !0 }), w = Number(P.total ?? 0) || 1, w === 1 ? j = "Fej" : w >= 2 && w <= 4 ? j = "Törzs" : w === 5 || w === 6 ? j = "Karok" : j = "Lábak";
        }
    ...
    if (await g.toMessage({
      ...
      const $ = Math.max(0, m - i);
      await a.update({ "system.resources.morale": $ });
    }
    this.close();
  }
}
...
//# sourceMappingURL=roll-sheet-DNVXwNwW.mjs.map
