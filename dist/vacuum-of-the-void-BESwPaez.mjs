...
const { HTMLField: Nt, NumberField: q, SchemaField: P, StringField: v, BooleanField: U, ArrayField: ue } = foundry.data.fields;
...
const { BooleanField: Kt, StringField: de } = foundry.data.fields;
class Ut extends foundry.abstract.TypeDataModel {
  static defineSchema() {
    return {
      ...
      protectionBonus: new de({ required: !1, blank: !0, initial: "" }),
      /** Sebesség levonás (a táblázatban: Sebesség Levonás). */
      speedPenalty: new de({ required: !1, blank: !0, initial: "" }),
      /** Szint (a táblázatban: Szint). */
      level: new de({ required: !1, blank: !0, initial: "" }),
...
const x = class x extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(i) {
    var Ae, Ee, Ne, $e, xe, je, Re, Me, Ke, Ue, Be, He, Ge, ze, We, Ye, Qe, Je;
    ...
    const pe = (((Qe = this.actor.items) == null ? void 0 : Qe.contents) ?? []).filter((d) => d.type === "MikroChip"), ce = pe.map((d) => ({ id: d.id, name: d.name, img: d.img })), qe = "— Nincs Mikro-Chip —", _e = ["slot1", "slot2", "slot3"], fe = (d) => {
      var I, O, L;
      const T = [];
    ...
    e.microchipItemsSlot1 = ce.filter((d) => !fe(1).includes(d.id)), e.microchipItemsSlot2 = ce.filter((d) => !fe(2).includes(d.id)), e.microchipItemsSlot3 = ce.filter((d) => !fe(3).includes(d.id)), e.microchipSlots = _e.map((d) => {
      ...
      const T = `slot${d}`, I = _[T] ?? {}, O = (I.itemId ?? "").trim(), L = ce.find((K) => K.id === O);
  ...
  _attachFrameListeners(i) {
    ...
    if ($(document).off("click.votv-weapon-dropdown"), $(document).off("click.votv-microchip-dropdown"), t.on("click", ".karakter-weapon-attack", async (r) => {
      ...
      if (!l) return;
      const { openRollSheetForWeapon: o } = await import("./roll-sheet-BIS25JHV.mjs");
      o(this.actor, l);
    ...
    p && n({ currentTarget: p }), this._writeEffectiveGivenSpeed(i), this._writeEffectiveGivenSpeed(e), this._writeEffectiveGivenProtection(i), this._writeEffectiveGivenProtection(e), requestAnimationFrame(() => {
      ...
      var g;
      r.preventDefault();
      const u = r.currentTarget, l = u.dataset.skill, o = ((g = u.textContent) == null ? void 0 : g.trim()) || l, { openRollSheetForSkill: c } = await import("./roll-sheet-BIS25JHV.mjs");
      c(this.actor, l, o);
...
//# sourceMappingURL=vacuum-of-the-void-BESwPaez.mjs.map
