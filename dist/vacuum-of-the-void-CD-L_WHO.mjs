...
var P = (E, r, e) => et(Ve(E), e, r);
const { HTMLField: rt, NumberField: w, SchemaField: _, StringField: f, BooleanField: j, ArrayField: oe } = foundry.data.fields;
...
const C = class C extends foundry.applications.api.HandlebarsApplicationMixin(
  ...
  async _prepareContext(r) {
    var Ie, Se, qe, Te, De, Oe, _e, Ae, Le, Fe, Ne, Ee, $e, Ce, xe, Pe, je;
    ...
    e.weaponsTable = te.map((d) => {
      const k = m[d] ?? {}, q = (k.itemId ?? "").trim(), T = N.find((se) => se.id === q), D = (T == null ? void 0 : T.system) ?? {}, x = typeof D.range == "string" ? (D.range || "").trim() : "", K = D.type ?? "", J = (U[K] ?? K) || "—", X = D.size ?? "", Q = (H[X] ?? X) || "", ae = [J, Q].filter(Boolean).join(", ") || J || "—", V = J === "Lövedékes", ee = Number(D.quantity ?? 1) || 1;
      return {
        slotKey: d,
        itemId: q,
    ...
    const re = (((Pe = this.actor.items) == null ? void 0 : Pe.contents) ?? []).filter((d) => d.type === "microchip"), ie = re.map((d) => ({ id: d.id, name: d.name, img: d.img }));
    e.microchipsTable = ["1", "2", "3"].map((d) => {
      var J, X;
      const k = `slot${d}`, q = b[k] ?? {}, T = (q.itemId ?? "").trim(), D = re.find((Q) => Q.id === T), x = ((J = D == null ? void 0 : D.system) == null ? void 0 : J.abilityType) ?? "", K = x === "active" ? "Aktív" : x === "passive" ? "Passzív" : x || "—";
      return {
        slotKey: k,
        itemId: T,
    ...
    const he = this.actor.system.abilities ?? {}, Xe = (((je = this.actor.items) == null ? void 0 : je.contents) ?? []).map((d) => {
      var k, q, T;
      return {
        id: d.id,
        name: d.name,
  ...
  _attachFrameListeners(r) {
    ...
    n && a({ currentTarget: n }), this._writeEffectiveGivenSpeed(r), this._writeEffectiveGivenSpeed(e), requestAnimationFrame(() => this._writeEffectiveGivenSpeed()), setTimeout(() => this._writeEffectiveGivenSpeed(), 100), t.on("click", ".karakter-portrait-img", (i) => {
      ...
      var h;
      i.preventDefault();
      const s = i.currentTarget, l = s.dataset.skill, c = ((h = s.textContent) == null ? void 0 : h.trim()) || l, { openRollSheetForSkill: m } = await import("./roll-sheet-D6pE7QMr.mjs");
      m(this.actor, l, c);
    ...
    if (e) {
      ...
      if (F) {
        ...
        if (b && b.documentName === "Item" && b.type === "Fegyver") {
          ...
          const O = L.system.weapons ?? {}, S = (O.slotOrder ?? "").trim(), M = ["slot1", "slot2", "slot3", "slot4", "slot5", "slot6", "slot7", "slot8", "slot9", "slot10"], N = S ? S.split(/\s*,\s*/).filter((U) => M.includes(U)) : [];
          if (N.find((U) => {
            var H;
          ...
          if (!Y) {
            const U = new Set(N), H = M.find((re) => !U.has(re));
            if (!H) return;
            Y = H, N.push(H);
          }
          const te = {
...
//# sourceMappingURL=vacuum-of-the-void-CD-L_WHO.mjs.map
